...
/**
 * @deprecated MapLibre does not require an API key. This function is kept for
 * backward compatibility with any code that references `mapboxApiKey` in the
 * payload data shape. It always returns an empty string.
 */
export function getMapboxApiKey(_mapboxApiKey?: string): string {
  return '';
}
