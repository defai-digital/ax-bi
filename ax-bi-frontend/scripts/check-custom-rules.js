 ...
 *
 * @param {string[]} candidateFiles paths relative to `ax-bi-frontend/`
 */
...
function normalizeCliFile(file) {
  return file.replace(/^ax-bi-frontend\//, '');
}
...
function main() {
  ...
  if (files.length === 0) {
    ...
    // Filter to only JS/TS files and remove ax-bi-frontend prefix
    files = files
