...
export default defineConfig({
  ...
  // Global test setup
  use: {
    // Use environment variable for base URL in CI, default to localhost:31423 for local
    // Normalize to always end with '/' to prevent URL resolution issues with APP_PREFIX
    baseURL: (() => {
      const url = process.env.PLAYWRIGHT_BASE_URL || 'http://localhost:31423';
  ...
  webServer: process.env.CI
    ...
    : (() => {
        ...
        const baseUrl =
          process.env.PLAYWRIGHT_BASE_URL || 'http://localhost:31423';
