...
import AxBIClient from '../../connection/AxBIClient';
import AxBIClientClass from '../../connection/AxBIClientClass';
import type {
  AxBIClientInterface,
  RequestConfig,
...
export default class ChartClient {
  readonly client: AxBIClientInterface | AxBIClientClass;
  ...
  constructor(
    ...
    const { client = AxBIClient } = config;
    this.client = client;
  }
  ...
  loadDatasource(
    ...
    return this.client
      .get({
        endpoint: `/ax-bi/fetch_datasource_metadata?datasourceKey=${datasourceKey}`,
        ...options,
      } as RequestConfig)
