...
import { axbiTheme, AxBITheme } from '@ax-bi/core/theme';
...
function SuperChart({
  ...
  // so we fall back to the default axbiTheme to avoid passing an invalid theme downstream.
  const themeFromContext = useTheme() as Partial<AxBITheme>;
  const theme =
    themeProp ??
    (Object.keys(themeFromContext).length > 0
      ? (themeFromContext as AxBITheme)
      : axbiTheme);
