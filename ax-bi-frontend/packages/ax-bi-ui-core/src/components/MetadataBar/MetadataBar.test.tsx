 ...
 * under the License.
 */
import { render, screen, userEvent, within } from '@ax-bi/ui-core/spec';
import { hexToRgb } from '@ax-bi/ui-core';
...
test('renders clickable items with blue icons when the bar is collapsed', async () => {
  await runWithBarCollapsed(async () => {
    ...
    expect(clickableColor).toBe(hexToRgb(axbiTheme.colorPrimary));
    expect(nonClickableColor.replace(/\s+/g, '')).toBe(
      axbiTheme.colorTextTertiary.replace(/\s+/g, ''),
    );
  });
});
