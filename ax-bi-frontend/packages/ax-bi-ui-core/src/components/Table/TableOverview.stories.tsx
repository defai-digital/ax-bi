...
Common table applications in AxBI:
...
\`\`\`
import { alphabeticalSort } from '@ax-bi/ui-core/components/Table/sorters';
...
you can register \`onDragOver\` and \`onDragDrop\` event handlers on the destination component. In the \`onDragDrop\` handler you can check for \`AXBI_TABLE_COLUMN\`
as the getData key as shown below.

\`\`\`
import { AXBI_TABLE_COLUMN } from '@ax-bi/ui-core/components/table';

const handleDrop = (ev:Event) => {
  const json = ev.dataTransfer?.getData?.(AXBI_TABLE_COLUMN);
