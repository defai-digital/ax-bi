...
export interface AxBIClientInterface extends Pick<
  AxBIClientClass,
  | 'delete'
  | 'get'
  | 'post'
  ...
  | 'getUrl'
> {
  configure: (config?: ClientConfig) => AxBIClientInterface;
  reset: () => void;
  getCSRFToken: () => CsrfPromise;
  guestTokenHeaderName?: string;
}

export type AxBIClientResponse = Response | JsonResponse | TextResponse;
