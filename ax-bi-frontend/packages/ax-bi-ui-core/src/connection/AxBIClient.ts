...
import AxBIClientClass from './AxBIClientClass';
import { AxBIClientInterface } from './types';

// this is local to this file, don't expose it
let singletonClient: AxBIClientClass | undefined;

function getInstance(): AxBIClientClass {
  if (!singletonClient) {
    throw new Error(
      'You must call AxBIClient.configure(...) before calling other methods',
    );
  }
  return singletonClient;
}

const AxBIClient: AxBIClientInterface = {
  configure: config => {
    singletonClient = new AxBIClientClass(config);
    return AxBIClient;
  },
  reset: () => {
