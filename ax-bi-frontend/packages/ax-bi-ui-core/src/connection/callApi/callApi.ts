...
export default async function callApi({
  ...
  if (
    ...
    let axbiCache: Cache | null = null;
    try {
      axbiCache = await caches.open(CACHE_KEY);
      const cachedResponse = await axbiCache.match(url);
      if (cachedResponse) {
      ...
      // If axbi is in an iframe and third-party cookies are disabled, caches.open throws
    }

    const response = await fetchWithRetry(url, request);

    if (axbiCache && response.status === HTTP_STATUS_NOT_MODIFIED) {
      const cachedFullResponse = await axbiCache.match(url);
    ...
    if (
      ...
      axbiCache.delete(url);
      axbiCache.put(url, response.clone());
    }
