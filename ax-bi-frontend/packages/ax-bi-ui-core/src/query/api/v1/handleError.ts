...
import {
  AxBIApiError,
  AxBIApiErrorPayload,
  AxBIApiMultiErrorsPayload,
} from './types';

export type ErrorInput = string | Error | Response | AxBIApiErrorPayload;

/**
 * Handle API request errors, convert to consistent AxBI API error.
 * @param error the catched error from AxBIClient.request(...)
 */
export default async function handleError(error: ErrorInput): Promise<never> {
  // already a AxBI error
  if (error instanceof AxBIApiError) {
    throw error;
  }
  // string is the error message itself
  if (typeof error === 'string') {
    throw new AxBIApiError({ message: error });
  }
  // JS errors, normally happens before request was sent
  if (error instanceof Error) {
    throw new AxBIApiError({
  ...
  if (error instanceof Response) {
    ...
    try {
      errorJson = (await error.json()) as
        AxBIApiErrorPayload | AxBIApiMultiErrorsPayload;
