...
interface AxBIApiFactoryOptions extends Omit<RequestBase, 'url'> {
  /**
...
>(
  ...
  async function request(
    payload: Payload,
    { client = AxBIClient }: AxBIApiRequestOptions = {
      client: AxBIClient,
    },
  ): Promise<Result> {
    try {
      ...
      if (responseType === 'text') {
        ...
        if (result && typeof result === 'object' && 'error' in result) {
          return handleError(result as AxBIApiErrorPayload);
        }
      }
