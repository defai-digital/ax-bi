...
describe('shared modules', () => {
  ...
  test('assigns to window', async () => {
    ...
    expect((window as any)['__axbi__/test-module']).toStrictEqual(fakeModule);
  });
  ...
  test('shares a map of modules', async () => {
    ...
    expect((window as any)['__axbi__/module-foo']).toEqual(foo);
    expect((window as any)['__axbi__/module-fizz']).toEqual(fizz);
  });
});
