...
import { CallApi, JsonObject } from '@ax-bi/ui-core';
...
describe('callApi()', () => {
  ...
  describe('caching', () => {
    ...
    test('caches requests with ETags', async () => {
      ...
      expect(calls).toHaveLength(1);
      const axbiCache = await caches.open(constants.CACHE_KEY);
      const cachedResponse = await axbiCache.match(mockCacheUrl);
