...
import { JsonValue, AxBIClientClass } from '@ax-bi/ui-core';
import { makeApi, AxBIApiError } from '../../../../src/query';
...
describe('makeApi()', () => {
  ...
  test('should allow custom path', async () => {
    ...
    const client = new AxBIClientClass({ appRoot: '/foo' });
  ...
  test('should handle errors', async () => {
    ...
    try {
      ...
      expect((error as AxBIApiError).message).toContain('Invalid payload');
    }
  });
  ...
  test('should handle error on 200 response', async () => {
    ...
    try {
      ...
      expect((error as AxBIApiError).message).toContain('not ok');
    }
  });
