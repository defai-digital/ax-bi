 ...
 * under the License.
 */
import { JsonObject, AxBIApiError, AxBIApiErrorType } from '@ax-bi/ui-core';
import handleError, {
  ErrorInput,
...
): Promise<AxBIApiError> {
  try {
    await handleError(inputError);
  } catch (error) {
    const typedError = error as AxBIApiError;
    expect(typedError).toBeInstanceOf(AxBIApiError);
...
describe('handleError()', () => {
  test('should throw error directly', async () => {
    expect.assertions(3);
    const input = new AxBIApiError({ message: 'timeout' });
  ...
  test('should handle response of single error', async () => {
    ...
    const mockResponse = new Response(
      '{ "error": "BAD BAD", "link": "https://github.com/defai-digital/ax-bi" }',
      {
        status: 403,
    ...
    await testHandleError(mockResponse, {
      message: 'BAD BAD',
      extra: { link: 'https://github.com/defai-digital/ax-bi' },
    });
  });
  ...
  test('should handle single error object', async () => {
    ...
    const mockError = {
      error: {
        ...
        error_type: AxBIApiErrorType.FrontendTimeoutError,
      },
    };
