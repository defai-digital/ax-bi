...
export class Theme {
  ...
  private constructor({ config }: { config?: AnyThemeConfig }) {
    this.AxBIThemeProvider = this.AxBIThemeProvider.bind(this);
  ...
  setConfig(config: AnyThemeConfig): void {
    ...
    // Extract AxBI-specific properties from top-level config.
    // These are custom properties that aren't part of Ant Design's token system
    // but need to be passed through to the AxBITheme for ECharts customization.
    ...
    this.theme = {
      ...
      // Include AxBI-specific properties from top-level config
  ...
  private updateProviders(
    ...
    // Overridden at runtime by AxBIThemeProvider using setThemeState
  }

  AxBIThemeProvider({ children }: { children: React.ReactNode }) {
    ...
    const [themeState, setThemeState] = React.useState({
      ...
      emotionCache: createCache({ key: 'axbi' }),
    });
