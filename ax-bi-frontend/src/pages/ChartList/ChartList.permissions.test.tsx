...
describe('ChartList - Permission-based UI Tests', () => {
  ...
  test('shows all UI elements for admin users with full permissions', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /^(?:plus\s*)?Chart$/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /import charts/i }),
    ).toBeInTheDocument();
  ...
  test('renders basic UI for anonymous users without permissions', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: /^(?:plus\s*)?Chart$/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /import charts/i }),
    ).not.toBeInTheDocument();
  });
