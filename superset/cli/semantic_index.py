...
def backfill_datasets(
    ...
    if dataset_ids:
        ...
        query = db.session.query(SqlaTable.id).order_by(SqlaTable.id.asc())
        if limit:
...
def _generation_cases(
    ...
    from superset.semantic_index.governance import load_dataset_eval_cases
...
def _generation_fn(dataset: Any) -> Any:
    ...
    from superset.mcp_service.ai.llm_provider import StubLLMProvider
    ...
    def generate(prompt: str) -> dict[str, Any] | None:
        if not isinstance(provider, StubLLMProvider):
            from superset.mcp_service.ai.intent_mapper import map_intent_to_chart
