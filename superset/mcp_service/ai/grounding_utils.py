...
def resolve_name_via_grounding(
    name: str,
    contract: Any | None,
    *,
    prefer: str = "any",
...
def enrich_dataset_plan_entry(ds_dict: dict[str, Any], dataset: Any | None) -> dict[str, Any]:
    ...
    if summary:
        ...
        if summary.get("measures") and not metrics:
            enriched["metrics"] = [m["name"] for m in summary["measures"] if m.get("name")]
    return enriched
...
def plan_should_block_compose(
    ...
    *,
    ...
    if chart_intent_count <= 0:
        return True, "Plan has no chart intents. Clarify the request or pin dataset_ids."
