...
def _time_columns(dataset: Any) -> list[str]:
    cols: list[str] = []
    main = getattr(dataset, "main_dttm_col", None)
    if main:
...
def resolve_metric_ref(name: str, dataset: Any) -> dict[str, Any]:
...
def _normalize_chart_type(chart_type: str | None) -> tuple[str, str]:
