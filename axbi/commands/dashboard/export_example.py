...
axbi/examples/ and loaded via the example loading system.
"""
...
def get_referenced_tables(sql: str, engine: str = "base") -> set[Table]:
    """Extract table references from SQL using AxBI's SQL parser.
...
def export_dashboard_yaml(
    ...
    from axbi.utils import (
        json as axbi_json,  # pylint: disable=import-outside-toplevel
    )
    ...
    if dashboard.json_metadata:
        try:
            json_metadata = axbi_json.loads(dashboard.json_metadata)
