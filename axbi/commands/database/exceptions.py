...
class DatabaseTestConnectionFailedError(AxBIErrorsException):
    status = 422
    message = _("Connection failed, please check your connection settings")
...
class DatabaseTestConnectionUnexpectedError(AxBIErrorsException):
    status = 422
...
class NoValidatorConfigFoundError(AxBIErrorException):
    status = 422
    message = _("no SQL validator is configured")


class NoValidatorFoundError(AxBIErrorException):
    status = 422
    message = _("No validator found (configured for the engine)")


class ValidatorSQLError(AxBIErrorException):
    status = 422
...
class ValidatorSQL400Error(AxBIErrorException):
    status = 400
    message = _("Was unable to check your query")
...
class InvalidEngineError(AxBIErrorException):
    status = 422


class DatabaseOfflineError(AxBIErrorException):
    status = 422


class InvalidParametersError(AxBIErrorsException):
    status = 422
