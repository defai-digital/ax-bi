...
from axbi.connectors.sqla.models import SqlaTable, validate_stored_expression
...
class UpdateDatasetCommand(UpdateMixin, BaseCommand):
    ...
    def _validate_expressions(
        ...
        for idx, item in enumerate(items):
            ...
            except (AxBISecurityException, QueryClauseValidationException) as ex:
                message = (
                    ex.error.message
                    if isinstance(ex, AxBISecurityException)
                    else ex.message
                )
                exceptions.append(
                    ValidationError(
