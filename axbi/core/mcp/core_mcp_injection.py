...
that replaces the abstract functions in ax-bi-core during initialization.
"""
...
def create_tool_decorator(
    ...
    *,
    ...
    This combines FastMCP tool registration with optional AxBI authentication
    ...
    Args:
        ...
        protect: Whether to apply AxBI authentication (defaults to True)
    ...
    def decorator(func: F) -> F:
        try:
            ...
            if feature_flags:
                from axbi.mcp_service.auth import FEATURE_FLAGS_ATTR

                setattr(func, FEATURE_FLAGS_ATTR, tuple(feature_flags))

            # Conditionally apply authentication wrapper
            if protect:
                from axbi.mcp_service.auth import mcp_auth_hook
...
def create_prompt_decorator(
    ...
    *,
    ...
    This combines FastMCP prompt registration with optional AxBI authentication,
...
def initialize_core_mcp_dependencies() -> None:
    ...
    in axbi_core.api.mcp with concrete implementations.

    Also imports MCP service app to register all host tools BEFORE extension loading.
    """
    import axbi_core.mcp.decorators
    ...
    try:
        ...
        logger.info(
            ...
            "Install it with: pip install 'ax-bi[fastmcp]'"
        )
        return

    # Replace the abstract decorators with concrete implementations
    axbi_core.mcp.decorators.tool = create_tool_decorator
    axbi_core.mcp.decorators.prompt = create_prompt_decorator

    try:
        from flask import current_app, has_app_context

        if has_app_context():
            from axbi.mcp_service.flask_singleton import register_flask_app
    ...
    try:
        ...
        from axbi.mcp_service import app  # noqa: F401
