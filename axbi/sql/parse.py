...
from axbi.sql.dialects import DB2, Dremio, Firebolt, OpenSearch, Pinot, Vertica
...
def _normalized_generator(
    ...
    *,
    ...
    arguments. AxBI's sanitize / format paths normalize user SQL — they
...
# engines supported by AxBI this is `sqlglot.exp.Expression`, but there is a special
...
class BaseSQLStatement(Generic[InternalRepresentation]):
    ...
    The `engine` parameters comes from the `engine` attribute in a AxBI DB engine
    spec.
    """
...
class SQLStatement(BaseSQLStatement[exp.Expression]):
    ...
    def _parse(cls, script: str, engine: str) -> list[exp.Expression]:
        ...
        try:
            ...
            raise AxBIParseError(script, engine, **kwargs) from ex
        except sqlglot.errors.SqlglotError as ex:
            raise AxBIParseError(
                script,
                engine,
...
class KustoKQLStatement(BaseSQLStatement[str]):
    ...
    def _extract_tables_from_statement(
        ...
        logger.warning(
            ...
            "roles will not be enforced by AxBI in the database."
        )
        return set()
...
class SQLScript:
    ...
    # adds a lot of complexity to AxBI, so we should avoid adding new engines to
    # this data structure.
    special_engines = {
    ...
    def has_unparseable_statement(self) -> bool:
        ...
        AST whose table references AxBI can enumerate. This covers two
...
def process_jinja_sql(
    ...
    :raises AxBISecurityException: If SQLGlot is unable to parse the SQL statement
    :raises jinja2.exceptions.TemplateError: If the Jinjafied SQL could not be rendered
    """

    from axbi.jinja_context import (  # pylint: disable=import-outside-toplevel
