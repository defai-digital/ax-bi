...
methods defined in axbi_core.api.models.Database.
...
axbi_core.api.models for the public API contract.
"""
...
def execute_sql_with_cursor(
...
) -> list[tuple[str, AxBIResultSet | None, float, int]]:
    """
    ...
    results: list[tuple[str, AxBIResultSet | None, float, int]] = []
...
class SQLExecutor:
    ...
    See axbi_core.api.models.Database for the full public API documentation.
    """
    ...
    def execute(
        ...
        See axbi_core.api.models.Database.execute() for full documentation.
        """
        from axbi_core.queries.types import (
    ...
    def execute_async(
        ...
        See axbi_core.api.models.Database.execute_async() for full documentation.
        """
        from axbi_core.queries.types import (
            QueryOptions as QueryOptionsType,
            QueryResult as QueryResultType,
        ...
        if opts.dry_run:
            from axbi_core.queries.types import StatementResult
    ...
    def _check_security(self, script: SQLScript) -> None:
        ...
        :raises AxBISecurityException: If security checks fail
        """
    ...
    def _store_in_cache(
        ...
        :param opts: Query options
        """
        from axbi_core.queries.types import QueryStatus
    ...
    def _submit_query_to_celery(
        ...
        from axbi.utils.core import get_username
    ...
    def _get_async_query_status(query_id: int) -> Any:
        ...
        from axbi_core.queries.types import QueryStatus as QueryStatusType
    ...
    def _get_async_query_result(query_id: int) -> Any:
        ...
        if query.results_key:
            ...
            from axbi import results_backend_manager
            ...
            if results_backend is not None:
                ...
                if blob:
                    try:
                        from axbi.utils.core import zlib_decompress
    ...
    def _cancel_query(database: Database, query: Any) -> bool:
        ...
        """
        from axbi.constants import QUERY_CANCEL_KEY, QUERY_EARLY_CANCEL_KEY
