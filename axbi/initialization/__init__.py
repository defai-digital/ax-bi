...
from axbi.extensions.context import extension_context
from axbi.security import AxBISecurityManager
from axbi.semantic_layers.labels import database_connections_menu_label
from axbi.sql.parse import SQLGLOT_DIALECTS
from axbi.utils.core import is_test, pessimistic_connection_handling
from axbi.utils.log import DBEventLogger, get_event_logger_from_cfg_value
...
class AxBIAppInitializer:  # pylint: disable=too-many-public-methods
    def __init__(self, app: AxBIApp) -> None:
        super().__init__()
    ...
    @deprecated(details="use self.axbi_app instead of self.flask_app")  # type: ignore
    @property
    def flask_app(self) -> AxBIApp:
        return self.axbi_app
    ...
    def init_views(self) -> None:
        ...
        from axbi.advanced_data_type.api import AdvancedDataTypeRestApi
        from axbi.annotation_layers.annotations.api import AnnotationRestApi
        from axbi.annotation_layers.api import AnnotationLayerRestApi
        from axbi.async_events.api import AsyncEventsRestApi
        from axbi.cachekeys.api import CacheRestApi
        from axbi.charts.api import ChartRestApi
        from axbi.charts.data.api import ChartDataRestApi
        from axbi.css_templates.api import CssTemplateRestApi
        from axbi.dashboards.api import DashboardRestApi
        from axbi.dashboards.filter_state.api import DashboardFilterStateRestApi
        from axbi.dashboards.permalink.api import DashboardPermalinkRestApi
        from axbi.databases.api import DatabaseRestApi
        from axbi.datasets.api import DatasetRestApi
        from axbi.datasets.columns.api import DatasetColumnsRestApi
        from axbi.datasets.metrics.api import DatasetMetricRestApi
        from axbi.datasource.api import DatasourceRestApi
        from axbi.embedded.api import EmbeddedDashboardRestApi
        from axbi.embedded.view import EmbeddedView
        from axbi.explore.api import ExploreRestApi
        from axbi.explore.form_data.api import ExploreFormDataRestApi
        from axbi.explore.permalink.api import ExplorePermalinkRestApi
        from axbi.extensions.view import ExtensionsView
        from axbi.importexport.api import ImportExportRestApi
        from axbi.queries.api import QueryRestApi
        from axbi.queries.saved_queries.api import SavedQueryRestApi
        from axbi.reports.api import ReportScheduleRestApi
        from axbi.reports.logs.api import ReportExecutionLogRestApi
        from axbi.row_level_security.api import RLSRestApi
        from axbi.security.api import (
            RoleRestAPI,
            SecurityRestApi,
            UserRegistrationsRestAPI,
        )
        from axbi.sqllab.api import SqlLabRestApi
        from axbi.sqllab.permalink.api import SqlLabPermalinkRestApi
        from axbi.tags.api import TagRestApi
        from axbi.themes.api import ThemeRestApi
        from axbi.views.alerts import AlertView, ReportView
        from axbi.views.all_entities import TaggedObjectsModelView
        from axbi.views.annotations import AnnotationLayerView
        from axbi.views.api import Api
        from axbi.views.chart.views import SliceModelView
        from axbi.views.core import AxBI
        from axbi.views.css_templates import CssTemplateModelView
        from axbi.views.dashboard.views import (
            Dashboard,
            DashboardModelView,
        )
        from axbi.views.database.views import DatabaseView
        from axbi.views.datasource.views import DatasetEditor, Datasource
        from axbi.views.dynamic_plugins import DynamicPluginsView
        from axbi.views.error_handling import set_app_error_handlers
        from axbi.views.explore import ExplorePermalinkView, ExploreView
        from axbi.views.groups import GroupsListView
        from axbi.views.log.api import LogRestApi
        from axbi.views.logs import ActionLogView
        from axbi.views.redirect import RedirectView
        from axbi.views.roles import RolesListView
        from axbi.views.sql_lab.views import (
            SavedQueryView,
            TableSchemaView,
            TabStateView,
        )
        from axbi.views.sqla import (
            RowLevelSecurityView,
            TableModelView,
        )
        from axbi.views.sqllab import SqllabView
        from axbi.views.tags import TagModelView, TagView
        from axbi.views.tasks import TaskModelView
        from axbi.views.themes import ThemeModelView
        ...
        from axbi.views.health import health_blueprint

        self.axbi_app.register_blueprint(health_blueprint)
        ...
        if feature_flag_manager.is_feature_enabled("SEMANTIC_LAYERS"):
            from axbi.semantic_layers.api import (
        ...
        appbuilder.add_view(
            ...
            menu_cond=lambda: bool(
                current_app.config.get("AXBI_SECURITY_VIEW_MENU", True)
            ),
        )
        ...
        appbuilder.add_view_no_menu(AxBI)
        ...
        appbuilder.add_view(
            ...
            icon="fa-list-ol",
            menu_cond=lambda: (
                self.config["FAB_ADD_SECURITY_VIEWS"] and self.config["AXBI_LOG_VIEW"]
            ),
        )
    ...
    def check_secret_key(self) -> None:
        if self.config["SECRET_KEY"] == CHANGE_ME_SECRET_KEY:
            warning = (
                "A Default SECRET_KEY was detected, please use axbi_config.py "
                "to override it.\n"
                "Use a strong complex alphanumeric string and use a tool to help"
                " you generate \n"
                "a sufficiently random sequence, ex: openssl rand -base64 42 \n"
                "For more info, see: https://github.com/defai-digital/ax-bi/"
                "tree/main/docs/admin_docs/configuration"
            )
            if self.axbi_app.debug or self.axbi_app.config["TESTING"] or is_test():
    ...
    def check_guest_token_secret(self) -> None:
        ...
        if not feature_flag_manager.is_feature_enabled("EMBEDDED_AXBI"):
            return
        if (
        ...
        self._log_config_warning(
            "EMBEDDED_AXBI is enabled but GUEST_TOKEN_JWT_SECRET has not "
            "been changed from its default value.\n"
            "The default value is publicly known and must be replaced before "
            "running in production.\n"
            "Set a strong random value in axbi_config.py:\n"
    ...
    def check_async_query_secret(self) -> None:
        ...
        self._log_config_warning(
            ...
            "The default value is publicly known and must be replaced before "
            "running in production.\n"
            "Set a strong random value (at least 32 bytes) in axbi_config.py:\n"
    ...
    def register_request_handlers(self) -> None:
        ...
        register_password_change_enforcement(self.axbi_app)

        @self.axbi_app.after_request
        def apply_http_headers(response: Response) -> Response:
            """Applies the configuration's http headers to all responses"""
            # HTTP_HEADERS is deprecated, this provides backwards compatibility
            response.headers.extend(
                {
                    **self.axbi_app.config["OVERRIDE_HTTP_HEADERS"],
                    **self.axbi_app.config["HTTP_HEADERS"],
                }
            )

            for k, v in self.axbi_app.config["DEFAULT_HTTP_HEADERS"].items():
        ...
        @self.axbi_app.context_processor
    ...
    def check_and_warn_database_connection(self) -> None:
        ...
        try:
            with self.axbi_app.app_context():
    ...
    def configure_auth_provider(self) -> None:
        machine_auth_provider_factory.init_app(self.axbi_app)

    def configure_ssh_manager(self) -> None:
        ssh_manager_factory.init_app(self.axbi_app)

    def configure_stats_manager(self) -> None:
        stats_logger_manager.init_app(self.axbi_app)

    def setup_event_logger(self) -> None:
        _event_logger["event_logger"] = get_event_logger_from_cfg_value(
            self.axbi_app.config.get("EVENT_LOGGER", DBEventLogger())
        )
    ...
    def configure_cache(self) -> None:
        cache_manager.init_app(self.axbi_app)
        results_backend_manager.init_app(self.axbi_app)

    def configure_feature_flags(self) -> None:
        feature_flag_manager.init_app(self.axbi_app)
    ...
    def configure_fab(self) -> None:
        ...
        custom_sm = self.config["CUSTOM_SECURITY_MANAGER"] or AxBISecurityManager
        if not issubclass(custom_sm, AxBISecurityManager):
            raise Exception(  # pylint: disable=broad-exception-raised
                """Your CUSTOM_SECURITY_MANAGER must now extend AxBISecurityManager,
                 not FAB's security manager.
                 See [4565] in UPDATING.md"""
            )

        appbuilder.indexview = AxBIIndexView
        appbuilder.security_manager_class = custom_sm
        appbuilder.init_app(self.axbi_app, db.session)
    ...
    def configure_url_map_converters(self) -> None:
        ...
        from axbi.utils.url_map_converters import (
            ObjectTypeConverter,
            RegexConverter,
        )

        self.axbi_app.url_map.converters["regex"] = RegexConverter
        self.axbi_app.url_map.converters["object_type"] = ObjectTypeConverter
    ...
    def configure_middlewares(self) -> None:  # noqa: C901
        if self.config["ENABLE_CORS"]:
            ...
            CORS(self.axbi_app, **self.config["CORS_OPTIONS"])

        if self.config["ENABLE_PROXY_FIX"]:
            self.axbi_app.wsgi_app = ProxyFix(
                self.axbi_app.wsgi_app, **self.config["PROXY_FIX_CONFIG"]
            )
        ...
        if self.config["ENABLE_CHUNK_ENCODING"]:
            ...
            self.axbi_app.wsgi_app = ChunkedEncodingFix(self.axbi_app.wsgi_app)

        if self.config["UPLOAD_FOLDER"]:
            with contextlib.suppress(OSError):
                os.makedirs(self.config["UPLOAD_FOLDER"])
        for middleware in self.config["ADDITIONAL_MIDDLEWARE"]:
            self.axbi_app.wsgi_app = middleware(self.axbi_app.wsgi_app)

        # Flask-Compress
        Compress(self.axbi_app)

        # Talisman
        talisman_enabled = self.config["TALISMAN_ENABLED"]
        talisman_config = (
            self.config["TALISMAN_DEV_CONFIG"]
            if self.axbi_app.debug or self.config["DEBUG"]
            else self.config["TALISMAN_CONFIG"]
        )
        csp_warning = self.config["CONTENT_SECURITY_POLICY_WARNING"]

        if talisman_enabled:
            talisman.init_app(self.axbi_app, **talisman_config)

        show_csp_warning = False
        if (
            csp_warning
            and not self.axbi_app.debug
            and (
    ...
    def configure_logging(self) -> None:
        self.config["LOGGING_CONFIGURATOR"].configure_logging(
            self.config, self.axbi_app.debug
        )

    def configure_db_encrypt(self) -> None:
        encrypted_field_factory.init_app(self.axbi_app)

    def setup_db(self) -> None:
        db.init_app(self.axbi_app)

        with self.axbi_app.app_context():
            pessimistic_connection_handling(db.engine)

        migrate.init_app(self.axbi_app, db=db, directory=APP_DIR + "/migrations")
    ...
    def setup_soft_delete_listener(self) -> None:
        ...
        from axbi.models.helpers import _add_soft_delete_filter
    ...
    def configure_async_queries(self) -> None:
        if feature_flag_manager.is_feature_enabled("GLOBAL_ASYNC_QUERIES"):
            ...
            if (
                ...
                return
            async_query_manager_factory.init_app(self.axbi_app)
    ...
    def configure_task_manager(self) -> None:
        ...
        if feature_flag_manager.is_feature_enabled("GLOBAL_TASK_FRAMEWORK"):
            ...
            TaskManager.init_app(self.axbi_app)
    ...
    def register_blueprints(self) -> None:
        ...
        for bp in self.config["BLUEPRINTS"]:
            try:
                ...
                self.axbi_app.register_blueprint(bp)
            except Exception:  # pylint: disable=broad-except
                logger.exception("blueprint registration failed")

    def setup_bundle_manifest(self) -> None:
        manifest_processor.init_app(self.axbi_app)

    def enable_profiling(self) -> None:
        if self.config["PROFILING"]:
            profiling.init_app(self.axbi_app)


class AxBIIndexView(IndexView):
    @expose("/")
    def index(self) -> FlaskResponse:
        return redirect(url_for("AxBI.welcome"))
