...
# under the License.
"""
A SQLAlchemy dialect for querying across AxBI databases.

The dialect ``axbi://`` allows users to query any table in any database that has been
configured in AxBI, eg:
...
from axbi import db, feature_flag_manager, security_manager
from axbi.sql.parse import Table


# pylint: disable=abstract-method
class AxBIAPSWDialect(APSWDialect):
    """
    A SQLAlchemy dialect for an internal AxBI engine.

    This dialect allows query to be executed across different AxBI
    ...
    Queries can also join data across different AxBI databases.

    The dialect is built in top of the Shillelagh library, leveraging SQLite to
    create virtual tables on-the-fly proxying AxBI tables. The
    `AxBIShillelaghAdapter` adapter is responsible for returning data when a
    AxBI table is accessed.
    """
...
def check_dml(method: F) -> F:
    ...
    @wraps(method)
    def wrapper(self: AxBIShillelaghAdapter, *args: Any, **kwargs: Any) -> Any:
...
class AxBIShillelaghAdapter(Adapter):
    """
    A Shillelagh adapter for AxBI tables.
    ...
    def supports(
        ...
        **kwargs: Any,
        ...
        eg, `axbi.examples.birth_names`.

        When using the AxBI SQLAlchemy and DB engine spec the prefix is dropped, so
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not feature_flag_manager.is_feature_enabled("ENABLE_AXBI_META_DB"):
            raise ProgrammingError("AxBI meta database is disabled")
    ...
    def _set_columns(self) -> None:
        ...
        # too: the referenced table must resolve to a AxBI dataset the
    ...
    def get_data(
        ...
        **kwargs: Any,
        ...
        """
        app_limit: int | None = current_app.config["AXBI_META_DB_LIMIT"]
        if limit is None:
            limit = app_limit
    ...
    def insert_row(self, row: Row) -> int:
        ...
        with self.engine_context() as engine, engine.begin() as connection:
