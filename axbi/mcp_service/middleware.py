...
from axbi.mcp_service.constants import (
    DEFAULT_TOKEN_LIMIT,
    DEFAULT_WARN_THRESHOLD_PCT,
)
from axbi.mcp_service.utils.logging_utils import mcp_event_log as _mcp_event_log
...
def _sanitize_error_for_logging(error: Exception) -> str:
    ...
    error_str = re.sub(
        r"/[a-zA-Z0-9_\-/.]{1,200}/ax-bi/", "/[REDACTED]/ax-bi/", error_str
    )
...
def _is_user_error(error: Exception) -> bool:
    ...
    if isinstance(error, _USER_ERROR_TYPES):
        return True
    # AxBIException and CommandException have a .status attribute.
    # 4xx = user error, 5xx = system error.
    if isinstance(error, AxBIException):
...
class GlobalErrorHandlerMiddleware(Middleware):
    ...
    async def _handle_error(  # noqa: C901
        ...
        elif isinstance(error, ForbiddenError | AxBISecurityException):
            # AxBI access denied — agent tried a tool it can't use
            raise ToolError(
                f"Permission denied for {tool_name}: "
                f"{_sanitize_error_for_logging(error)}"
            ) from error
        elif isinstance(error, AxBIException):
            # Other AxBI errors — .status determines severity (already
...
class RateLimitMiddleware(Middleware):
    ...
    def _resolve_principal(self, context: MiddlewareContext) -> tuple[str, bool]:
        ...
        Tries, in order: an authenticated AxBI user id (set when an
...
class FieldPermissionsMiddleware(Middleware):
    ...
    def _filter_response(self, response: Any, object_type: str, user: Any) -> Any:
        ...
        Returns:
            Filtered response
        """
        from axbi.mcp_service.utils.permissions_utils import filter_sensitive_data
...
class ResponseSizeGuardMiddleware(Middleware):
    ...
    Configuration via MCP_RESPONSE_SIZE_CONFIG in axbi_config.py:
...
def create_rate_limit_middleware() -> RateLimitMiddleware | None:
    ...
    try:
        from axbi.mcp_service.flask_singleton import get_flask_app
        from axbi.mcp_service.mcp_config import MCP_RATE_LIMIT_CONFIG
        from axbi.mcp_service.utils.config_utils import (
