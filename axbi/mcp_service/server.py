...
from axbi.mcp_service.app import create_mcp_app, init_fastmcp_server
from axbi.mcp_service.jwt_verifier import BrowserHelloMiddleware
...
def _suppress_third_party_warnings() -> None:
    ...
    # (axbi/mcp_service/__init__.py), but add it here too for any late
...
def _build_starlette_middleware(
    ...
    if app_icon := get_axbi_app_icon(flask_app.config):
        if app_icon.startswith(("http://", "https://")):
            base_page_config["logo_url"] = app_icon
        elif app_icon.startswith("/"):
            # Relative path — combine with AxBI webserver address if configured
            axbi_addr = get_axbi_webserver_address(flask_app.config).rstrip("/")
            if axbi_addr:
                base_page_config["logo_url"] = f"{axbi_addr}{app_icon}"
