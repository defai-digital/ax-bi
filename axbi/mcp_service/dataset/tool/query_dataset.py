...
from axbi.mcp_service.chart.schemas import DataColumn, PerformanceMetadata
...
from axbi.mcp_service.utils import _is_uuid
...
from axbi.mcp_service.utils.oauth2_utils import build_oauth2_redirect_message
...
async def query_dataset(  # noqa: C901
    ...
    try:
        ...
        with mcp_event_log_context(action="mcp.query_dataset.execute"):
            ...
            # datasets (AxBI's built-in semantic layer). External semantic
        ...
        await ctx.error(f"OAuth2 error: {str(exc)}")
        return DatasetError.create(
            error=f"OAuth2 authentication error: {exc}",
            error_type="OAuth2Error",
        )

    except (CommandException, AxBIException) as exc:
        await ctx.error(f"Query failed: {str(exc)}")
        return DatasetError.create(
        ...
        await ctx.error(f"Database error: {str(exc)}")
        return DatasetError.create(
        ...
        await ctx.error(f"Unexpected error: {type(exc).__name__}: {str(exc)}")
        return DatasetError.create(
