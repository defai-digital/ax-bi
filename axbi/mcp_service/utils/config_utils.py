...
def get_axbi_app_name(
    config: Mapping[str, Any] | None = None,
    default: str = "AX BI",
) -> str:
    """Read AxBI's configured app name behind the MCP config boundary."""
    source = config if config is not None else current_app.config
    return str(source.get("APP_NAME", default))


def get_axbi_app_icon(config: Mapping[str, Any] | None = None) -> Any:
    """Read AxBI's configured app icon URL/path."""
    source = config if config is not None else current_app.config
    return source.get("APP_ICON", "")


def get_axbi_admin_role_name(config: Mapping[str, Any] | None = None) -> Any:
    """Read AxBI's configured admin role name."""
    source = config if config is not None else current_app.config
    return source["AUTH_ROLE_ADMIN"]


def get_axbi_webserver_address(config: Mapping[str, Any] | None = None) -> Any:
    """Read AxBI's configured public webserver address."""
    source = config if config is not None else current_app.config
    return source.get("AXBI_WEBSERVER_ADDRESS") or os.environ.get(
        "AXBI_WEBSERVER_ADDRESS",
        "",
    )
...
def get_mcp_bug_report_contact(config: Mapping[str, Any] | None = None) -> Any:
    """Read the MCP bug-report support contact from AxBI config."""
    source = config if config is not None else current_app.config
...
def get_axbi_row_limit(config: Mapping[str, Any] | None = None) -> Any:
    """Read AxBI's configured row limit behind the MCP config boundary."""
    source = config if config is not None else current_app.config
    return source["ROW_LIMIT"]


def get_upload_max_file_size_bytes(config: Mapping[str, Any] | None = None) -> Any:
    """Read AxBI's upload file-size limit behind the MCP config boundary."""
    source = config if config is not None else current_app.config
