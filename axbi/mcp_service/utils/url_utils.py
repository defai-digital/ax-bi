...
def _browser_safe_base_url(url: str) -> str:
    ...
    if parsed.hostname != "0.0.0.0":  # noqa: S104 - URL comparison, not a bind
        return url
...
def get_axbi_base_url() -> str:
    """
    Get the AxBI base URL from configuration.

    Returns:
        Base URL for AxBI web server (e.g., "http://localhost:9001")
    """
    default_url = "http://localhost:9001"

    try:
        if webserver_url := _normalize_base_url(get_axbi_webserver_address()):
...
def get_mcp_service_url() -> str:
    ...
    AxBI URL with /mcp prefix. In development,
