...
from axbi.mcp_service.utils.config_utils import get_mcp_jwt_algorithm
...
_DEFAULT_HELLO_PAGE_CONFIG: dict[str, Any] = {
    ...
    "title": "AX BI MCP Server",
    # Key name used in the mcpServers config snippet (e.g. "axbi", "my-company")
    "server_key": "axbi",
...
def _build_config_snippet(
    ...
    # axbi.utils.json.dumps() ensures the key is a valid JSON string.
    from axbi.utils import json as axbi_json

    key_json = axbi_json.dumps(server_key)
...
def _build_browser_hello_html(
    ...
    app_name: str = html_module.escape(str(cfg.get("app_name", "AX BI")))
...
class MCPJWTVerifier(JWTVerifier):
    """JWTVerifier with AxBI MCP auth error handling.
    ...
    Use this as the base for all AxBI JWT verifiers so the browser hello
