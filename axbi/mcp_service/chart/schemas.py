...
from axbi.mcp_service.common.cache_schemas import (
    CacheStatus,
    CreatedByMeMixin,
...
def serialize_chart_object(chart: ChartLike | None) -> ChartInfo | None:
    ...
    chart_url = None
    if chart_id:
        chart_url = f"{get_axbi_base_url()}/explore/?slice_id={chart_id}"
...
class PieChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        None,
        description=(
            "AxBI color scheme ID (e.g. 'axbiColors', 'lyftColors', "
            "'googleCategory10c', 'd3Category10'). Defaults to 'axbiColors'."
        ),
        max_length=100,
    )
...
class MixedTimeseriesChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        ...
        description=(
            "AxBI color scheme ID (e.g. 'axbiColors', 'lyftColors'). "
            "When omitted, AxBI's default scheme is used."
        ),
        max_length=100,
    )
    currency_format: CurrencyFormat | None = Field(
...
class BigNumberChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        None,
        description=(
            "AxBI color scheme ID for the trendline (e.g. 'axbiColors'). "
            "When omitted, AxBI's default scheme is used."
        ),
        max_length=100,
    )
...
class TableChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        None,
        description=(
            "AxBI color scheme ID applied to conditional/cell formatting "
            "(e.g. 'axbiColors')."
        ),
        max_length=100,
    )
...
class XYChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        ...
        description=(
            "AxBI color scheme ID (e.g. 'axbiColors', 'lyftColors', "
            "'googleCategory10c', 'd3Category10'). When omitted, AxBI's "
        ...
        if self.group_by:
            for i, col in enumerate(self.group_by):
                if self.x is not None and col.name == self.x.name:
                    ...
                    # to prevent AxBI "duplicate label" errors, so
...
class ListChartsRequest(OwnedByMeMixin, CreatedByMeMixin, MetadataCacheControl):
    ...
    def parse_filters(cls, v: Any) -> list[ChartFilter]:
        ...
        """
        from axbi.mcp_service.utils.schema_utils import parse_filters
...
class GenerateChartRequest(QueryCacheControl):
    ...
    save_chart: bool = Field(default=False, description="Save permanently in AxBI")
...
class GenerateExploreLinkRequest(FormDataCacheControl):
    ...
    config: ChartConfig | None = Field(
        ...
        description=(
            ...
            "explore URL that opens the dataset in AxBI without a "
            "preconfigured chart."
        ),
    )
...
class GenerateChartResponse(BaseModel):
    ...
    explore_url: str | None = Field(None, description="Edit chart in AxBI")
...
class ChartPreview(BaseModel):
    ...
    explore_url: str = Field(description="URL to open chart in AxBI for editing")
