...
from axbi.mcp_service.chart.chart_utils import validate_chart_dataset
...
from axbi.mcp_service.utils.cache_utils import get_cache_status_from_result
from axbi.mcp_service.utils.config_utils import get_axbi_row_limit
...
# Maps AxBI viz_type strings to canonical categories so we can
...
async def get_chart_data(  # noqa: C901
    ...
    try:
        ...
        # https://github.com/defai-digital/ax-bi/issues/39206 for the analogous
        # database eager-load fix.
        ...
        try:
            ...
            if query_context_json is None and not using_unsaved_state:
                ...
                row_limit = (
                    request.limit or form_data.get("row_limit") or get_axbi_row_limit()
                )
                ...
                if all(
                    ...
                    return ChartError(
                        error=(
                            ...
                            f"Please open this chart in AxBI and "
        ...
        except (CommandException, AxBIException, ValueError) as data_error:
            await ctx.error(
...
async def _query_from_form_data(
    ...
    row_limit = request.limit or form_data.get("row_limit") or get_axbi_row_limit()
    ...
    except (CommandException, AxBIException, ValueError) as e:
...
def _export_data_as_csv(
    ...
    from axbi.mcp_service.chart.schemas import ChartData
