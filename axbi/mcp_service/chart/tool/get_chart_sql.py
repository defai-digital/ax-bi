...
def _sql_from_saved_query_context(
    ...
    except (AxBIException, CommandException, TypeError, ValueError):
        return None
...
def _resolve_datasource_name(
    ...
    try:
        ...
        from axbi.daos.exceptions import (
            DatasourceNotFound,
...
async def get_chart_sql(
    ...
    feature in the AxBI UI. Useful for understanding, debugging, or
    auditing the SQL generated by a chart's configuration.

    Note: The returned SQL is RLS-scoped to the caller and contains
    table/schema names. Same disclosure surface as the AxBI UI
    ...
    except AxBIException as e:
        logger.exception("AxBI error in get_chart_sql")
        return ChartError(
            error=f"AxBI error: {e}",
            error_type="AxBIError",
        )
...
async def _handle_chart_sql_request(
    ...
    with mcp_event_log_context(action="mcp.get_chart_sql.build_query"):
        ...
        except (AxBIException, CommandException, ValueError) as e:
