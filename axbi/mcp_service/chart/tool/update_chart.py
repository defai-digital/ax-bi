...
from axbi.mcp_service.chart.compile import validate_and_compile
...
from axbi.mcp_service.utils import escape_llm_context_delimiters
...
def _create_preview_url(
    ...
    from axbi.mcp_service.commands.create_form_data import MCPCreateFormDataCommand
...
async def update_chart(  # noqa: C901
    ...
    try:
        ...
        from axbi.mcp_service.auth import check_chart_data_access
        ...
        if not request.generate_preview:
            ...
            saved = True
            explore_url = f"{get_axbi_base_url()}/explore/?slice_id={updated_chart.id}"
        else:
        ...
        result = {
            ...
            "form_data_key": form_data_key,
            "api_endpoints": {
                "data": f"{get_axbi_base_url()}/api/v1/chart/{chart_id}/data/",
                "export": (f"{get_axbi_base_url()}/api/v1/chart/{chart_id}/export/"),
            },
        ...
        await ctx.warning(
            f"Chart update requires OAuth authentication: "
        ...
        await ctx.error(f"OAuth2 configuration error: chart_id={request.identifier}")
        return GenerateChartResponse.model_validate(
            {
                "chart": None,
                "success": False,
