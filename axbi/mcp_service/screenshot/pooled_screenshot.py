...
from axbi.mcp_service.screenshot.webdriver_pool import get_webdriver_pool
...
from axbi.mcp_service.utils.retry_utils import retry_screenshot_operation
from axbi.utils.screenshots import BaseScreenshot, WindowSize
...
class PooledChartScreenshot(PooledBaseScreenshot):
    ...
    def __init__(
        ...
        from axbi.utils.urls import modify_url_query
        from axbi.utils.webdriver import ChartStandaloneMode
    ...
    def _take_screenshot(self, driver: Any, user: User) -> bytes | None:
        """Take screenshot of chart with standard AxBI chart handling"""
        try:
...
class PooledDashboardScreenshot(PooledBaseScreenshot):
    ...
    def __init__(
        ...
        from axbi.utils.urls import modify_url_query
        from axbi.utils.webdriver import DashboardStandaloneMode
    ...
    def _take_screenshot(self, driver: Any, user: User) -> bytes | None:
        """Take screenshot of dashboard with standard AxBI dashboard handling"""
        try:
            # Wait for page to load
            selenium_headstart = get_screenshot_selenium_headstart()
            time.sleep(selenium_headstart)
