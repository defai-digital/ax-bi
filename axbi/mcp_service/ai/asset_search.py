...
from axbi.runtime_modernization.rust_genai import (
...
def _search_datasets(
    ...
    search_filter = or_(*search_clauses) if search_clauses else false()
...
def _search_dashboards(
    ...
    search_filter = or_(*search_clauses) if search_clauses else Dashboard.id.is_(None)
...
def _filter_sidecar_results_by_access(
    ...
    AX Services authenticates to AxBI with a deployment-level internal
    ...
    while making AxBI's caller-scoped base filters authoritative.
    """
    if not results:
        return []
