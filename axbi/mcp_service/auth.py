...
- RBAC permission checking aligned with AxBI's REST API permissions
...
def _check_tool_rbac_permission(
    ...
    and uses AxBI's security_manager to verify access.

    Controlled by the ``MCP_RBAC_ENABLED`` config flag (default True).
    Set to False in axbi_config.py to disable RBAC checking.
...
def load_user_with_relationships(
    ...
    Delegates to :meth:`AxBISecurityManager.find_user_with_relationships`,
...
def _resolve_user_from_jwt_context(app: Any) -> User | None:
    ...
    # same username/email claim would resolve to the same AxBI user.
    #
    ...
    if (
        ...
        logger.warning(
            ...
            "the default user resolver maps token claims to AxBI users by "
    ...
    if not user:
        ...
        raise ValueError(
            "JWT authenticated user not found in AxBI database. "
...
def has_dataset_access(dataset: "SqlaTable") -> bool:
    ...
    has permission to access the given dataset using AxBI's security manager.
    ...
    try:
        ...
        if hasattr(g, "user") and g.user:
            # Use AxBI's security manager to check dataset access
...
def _get_app_context_manager() -> AbstractContextManager[None]:
    ...
    **new** app context so concurrent tool calls do not share one ``g``
    ...
    # AxBI is fully initialised (e.g. during unit-test collection).
