...
def register_flask_app(flask_app: Flask) -> None:
    """
    Register an already-initialized AxBI Flask app for MCP helpers.

    Main AxBI startup initializes Flask-AppBuilder inside an app context,
...
try:
    ...
    # Check if appbuilder is already initialized (main AxBI app is running).
    ...
    if appbuilder_initialized and has_app_context():
        # We're in an app context (e.g., during main AxBI startup),
        ...
        # Standalone MCP server — AxBI models are deeply coupled to
        ...
        # AxBI web server is not running in-process.
