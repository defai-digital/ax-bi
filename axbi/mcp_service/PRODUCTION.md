...
- ✅ **Access Control**: RBAC and RLS enforcement through AxBI's security manager
...
```python
MCP_JWT_ISSUER = "https://keycloak.yourcompany.com/realms/axbi"
MCP_JWT_AUDIENCE = "ax-bi-mcp"
MCP_JWKS_URI = "https://keycloak.yourcompany.com/realms/axbi/protocol/openid-connect/certs"
```
...
AxBI's existing RBAC is automatically enforced. Ensure roles are configured:

```python
# In AxBI UI: Security → List Roles
...
```python
# Future implementation - add to axbi_config.py
...
graph TB
    ...
    subgraph "Application Tier"
        ...
        MCP2[MCP Instance 2]
        MCP3[MCP Instance 3]
        AxBI[AxBI Web Server]
    end

    subgraph "Data Tier"
        DB[(PostgreSQL<br/>AxBI Metadata)]
...
python3.11 -m venv /opt/ax-bi/venv
source /opt/ax-bi/venv/bin/activate

# Install AxBI with MCP support
pip install ax-bi[fastmcp]
...
```python
# /opt/ax-bi/axbi_config.py

# Database connection
SQLALCHEMY_DATABASE_URI = "postgresql://user:pass@db-host:5432/axbi"
...
# AxBI webserver address (for screenshot generation)
AXBI_WEBSERVER_ADDRESS = "https://axbi.yourcompany.com"
WEBDRIVER_BASEURL = "https://axbi.yourcompany.com/"
...
```bash
# /opt/ax-bi/.env
export AXBI_CONFIG_PATH=/opt/ax-bi/axbi_config.py
...
User=axbi
Group=axbi
WorkingDirectory=/opt/ax-bi

Environment="AXBI_CONFIG_PATH=/opt/ax-bi/axbi_config.py"
Environment="FLASK_APP=axbi"

ExecStart=/opt/ax-bi/venv/bin/ax-bi mcp run --port 5008
...
[program:ax-bi-mcp]
command=/opt/ax-bi/venv/bin/ax-bi mcp run --port 5008
directory=/opt/ax-bi
user=axbi
autostart=true
autorestart=true
redirect_stderr=true
stdout_logfile=/var/log/axbi/mcp.log
stdout_logfile_maxbytes=50MB
stdout_logfile_backups=10
environment=AXBI_CONFIG_PATH="/opt/ax-bi/axbi_config.py",FLASK_APP="axbi"
```
...
```dockerfile
FROM ghcr.io/defai-digital/ax-bi:latest

# Install MCP dependencies
RUN pip install ax-bi[fastmcp]

# Copy production config
COPY axbi_config.py /app/pythonpath/
...
docker run -d \
  --name ax-bi-mcp \
  -p 5008:5008 \
  -v /opt/ax-bi/axbi_config.py:/app/pythonpath/axbi_config.py:ro \
  -e AXBI_CONFIG_PATH=/app/pythonpath/axbi_config.py \
  ax-bi-mcp:latest
    ...
    spec:
      ...
      - name: mcp
        ...
        - containerPort: 5008
          name: mcp
        env:
        - name: AXBI_CONFIG_PATH
          value: /app/pythonpath/axbi_config.py
        - name: FLASK_APP
          value: axbi
        volumeMounts:
        - name: config
...
```bash
# Create ConfigMap from axbi_config.py
kubectl create configmap axbi-config \
  --from-file=axbi_config.py=/opt/ax-bi/axbi_config.py
  ...
  "logs": {
    "logs_collected": {
      "files": {
        "collect_list": [
          {
            "file_path": "/var/log/axbi/mcp.log",
            "log_group_name": "/aws/axbi/mcp",
...
- [ ] AxBI webserver address updated for production URL
...
1. **Immediate Rollback**:
   ...
   cp /opt/ax-bi/axbi_config.py.backup /opt/ax-bi/axbi_config.py
   systemctl start ax-bi-mcp
...
**Diagnosis**:
```bash
# Check if user exists in AxBI
ax-bi fab list-users | grep username
```

**Solution**:
- Create user in AxBI: `ax-bi fab create-user`
- Ensure JWT `sub` claim matches AxBI username
...
# Check role permissions in AxBI UI
...
# AxBI webserver request timeout
AXBI_WEBSERVER_TIMEOUT = 60
```

## References

- **AxBI Configuration**: https://superset.apache.org/docs/configuration/configuring-axbi
- **AxBI Installation**: https://superset.apache.org/docs/installation/installing-axbi-from-scratch
