...
def _collect_tabs_groups(layout: dict[str, Any]) -> list[list[str]]:
    ...
    AxBI dashboards can place TABS under either ROOT_ID or GRID_ID
...
def _ensure_layout_structure(
    ...
    if "ROOT_ID" in layout:
        ...
        # child of ROOT_ID.  Real AxBI dashboards with tabs place a
...
def add_chart_to_existing_dashboard(  # noqa: C901 — complexity is structural (layout traversal + multi-step authorization), not accidental
    ...
    try:
        ...
        dashboard_info = DashboardInfo(
            ...
            url=f"{get_axbi_base_url()}/ax-bi/dashboard/{updated_dashboard.id}/",
        ...
        dashboard_url = f"{get_axbi_base_url()}/ax-bi/dashboard/{updated_dashboard.id}/"
