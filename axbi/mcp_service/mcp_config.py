...
from axbi.mcp_service.jwt_verifier import DetailedJWTVerifier, MCPJWTVerifier
...
# Note: MCP_DEV_USERNAME MUST be configured in axbi_config.py
...
# deployment should override this in axbi_config.py to point users at the
...
# user's AxBI administrator and the AX BI issue tracker.
...
# clients never see them. Use this when a AxBI-provided tool conflicts with
...
# AxBI logs a startup warning to make the implicit enablement visible.
...
def default_user_resolver(app: Any, access_token: Any) -> str | None:
    ...
    if isinstance(claims, dict) and claims:
        ...
        # (OIDC `sub` is often a stable opaque ID, not a AxBI username)
        username = (
...
def get_mcp_config(app_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    defaults = {
        "AXBI_WEBSERVER_ADDRESS": AXBI_WEBSERVER_ADDRESS,
...
def get_mcp_config_with_overrides(
    ...
    This version lets users add custom MCP config keys in axbi_config.py
