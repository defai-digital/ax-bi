...
The AX BI MCP (Model Context Protocol) service provides programmatic access to AxBI functionality through a standardized protocol that enables AI assistants and automation tools to interact with dashboards, charts, datasets, and SQL Lab.
...
- The MCP service runs as an independent process from the AxBI web server
...
**AxBI's Existing RLS (Row-Level Security)**:
...
- The MCP service respects all RLS rules automatically through AxBI's query execution layer

**No Schema-Based Isolation**:
- The current implementation does NOT use separate database schemas per tenant
- All AxBI metadata (dashboards, charts, datasets) exists in the same database schema
- Database-level isolation is achieved through AxBI's permission system rather than physical schema separation
...
- AxBI's security manager then enforces permissions based on this user's roles
...
- All list operations (dashboards, charts, datasets) use AxBI's access filters:
...
The MCP service uses SQLAlchemy's connection pooling with configuration inherited from AxBI:
...
**Shared Database with AxBI**:
- MCP service and AxBI web server share the same database
...
**Example Calculation**:
- AxBI web: 8 workers × 10 connections = 80
...
sequenceDiagram
    ...
    participant DAO as AxBI DAO Layer<br/>(ChartDAO, DashboardDAO)
...
graph TD
    ...
    DB[(AxBI Database<br/>shared connection pool)]
...
graph TD
    ...
    DB[(AxBI Database<br/>single schema, filtered by permissions)]
...
| Security | Depends on AxBI RBAC | Stronger isolation |
...
- **AxBI Security Model**: https://superset.apache.org/docs/security
