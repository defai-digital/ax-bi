...
DEFAULT_SUPPORT_CONTACT = (
    "your AxBI administrator or the AX BI community "
    "(https://github.com/defai-digital/ax-bi/issues)"
)
...
def _collect_environment() -> dict[str, str]:
    ...
    env: dict[str, str] = {
        ...
        "axbi_version": "unknown",
        "service": "AX BI MCP Service",
    }

    try:
        version_metadata = get_version_metadata()
        env["axbi_version"] = _safe_str(
            version_metadata.get("version_string", "unknown")
        )
    except Exception:  # noqa: BLE001
        logger.warning("bug_report: unable to read AxBI version", exc_info=True)

    try:
        app_name = get_axbi_app_name()
...
def _format_report(
    ...
    lines: list[str] = [
        "# AX BI MCP Bug Report",
        "",
        f"- **Timestamp (UTC):** {timestamp}",
        f"- **Service:** {environment['service']}",
        f"- **AX BI version:** {environment['axbi_version']}",
    ...
    lines += [
        ...
        "---",
        (
            "_This report was generated by the AX BI MCP service. "
...
async def generate_bug_report(
    ...
    ``MCP_BUG_REPORT_CONTACT`` setting in ``axbi_config.py`` so each
    deployment can point users at the right channel. The default points
    at the user's AxBI administrator and the AX BI issue
    tracker.
