...
This allows running the service with: python -m axbi.mcp_service
"""
...
def _add_default_middlewares() -> None:
    ...
    from axbi.mcp_service.middleware import create_response_size_guard_middleware
...
def main() -> None:
    ...
    parser = ArgumentParser(
        description="Run the AX BI MCP service.",
    )
    parser.parse_args()

    from axbi.mcp_service.app import init_fastmcp_server, mcp
    ...
    if transport == "stdio":
        ...
        # Disable all Flask/AxBI logging to stdout
        for logger_name in [
            "axbi",
            "flask",
            "werkzeug",
        ...
        from axbi.mcp_service.utils.config_utils import get_mcp_debug_enabled
