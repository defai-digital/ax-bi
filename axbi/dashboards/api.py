...
from axbi.charts.schemas import ChartEntityResponseSchema
...
from axbi.commands.dashboard.delete import (
...
from axbi.commands.dashboard.export_example import ExportExampleCommand
from axbi.commands.dashboard.fave import AddFavoriteDashboardCommand
from axbi.commands.dashboard.importers.dispatcher import ImportDashboardsCommand
...
from axbi.commands.database.exceptions import DatasetValidationError
from axbi.commands.exceptions import TagForbiddenError
from axbi.commands.importers.exceptions import NoValidFilesFoundError
from axbi.commands.importers.v1.utils import get_contents_from_bundle
from axbi.constants import MODEL_API_RW_METHOD_PERMISSION_MAP, RouteMethod
from axbi.daos.dashboard import DashboardDAO, EmbeddedDashboardDAO
from axbi.dashboards.filters import (
...
from axbi.security.guest_token import GuestUser
from axbi.tasks.thumbnails import (
    cache_dashboard_screenshot,
    cache_dashboard_thumbnail,
)
from axbi.tasks.utils import get_current_user
from axbi.utils.core import parse_boolean_string, sanitize_cookie_token
from axbi.utils.file import get_filename
...
def with_dashboard(
    f: Callable[[BaseAxBIModelRestApi, Dashboard], Response],
) -> Callable[[BaseAxBIModelRestApi, str], Response]:
    """
    ...
    def wraps(self: BaseAxBIModelRestApi, id_or_slug: str) -> Response:
        try:
...
class DashboardRestApi(CustomTagsOptimizationMixin, BaseAxBIModelRestApi):
    ...
    @permission_name("get")
    @merge_response_func(
        BaseAxBIModelRestApi.merge_order_columns, API_ORDER_COLUMNS_RIS_KEY
    )
    @merge_response_func(
        BaseAxBIModelRestApi.merge_list_label_columns, API_LABEL_COLUMNS_RIS_KEY
    )
    @merge_response_func(
        BaseAxBIModelRestApi.merge_description_columns,
        API_DESCRIPTION_COLUMNS_RIS_KEY,
    )
    @merge_response_func(
        BaseAxBIModelRestApi.merge_list_columns, API_LIST_COLUMNS_RIS_KEY
    )
    @merge_response_func(BaseAxBIModelRestApi.merge_list_title, API_LIST_TITLE_RIS_KEY)
    ...
    def __repr__(self) -> str:
        ...
        return "AxBI.dashboards.api.DashboardRestApi@v{}{}".format(
    ...
    def export_as_example(self, pk: int) -> Response:
        ...
        get:
          ...
          description: >-
            ...
            format used by the AxBI example loading system. The export
    ...
    def cache_dashboard_screenshot(self, pk: int, **kwargs: Any) -> WerkzeugResponse:
        ...
        ) and not security_manager.can_access("can_export_image", "AxBI"):
            return self.response_403()
        try:
        ...
        dashboard_url = get_url_path("AxBI.dashboard_permalink", key=permalink_key)
    ...
    def thumbnail(self, pk: int, digest: str, **kwargs: Any) -> WerkzeugResponse:
        ...
        dashboard_url = get_url_path(
            "AxBI.dashboard", dashboard_id_or_slug=dashboard.id
        )
