...
from axbi.axbi_typing import Column, QueryObjectDict
from axbi.commands.dataset.exceptions import DatasetNotFoundError
from axbi.common.utils.time_range_utils import get_since_until_from_time_range
from axbi.constants import LRU_CACHE_MAX_SIZE, NO_TIME_RANGE
...
class UndefinedTemplateFunctionException(AxBITemplateException):
...
class AxBISandboxedEnvironment(SandboxedEnvironment):
    """
...
class BaseTemplateProcessor:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.env: Environment = AxBISandboxedEnvironment(undefined=DebugUndefined)
    ...
    def process_template(self, sql: str, **kwargs: Any) -> str:
        ...
        try:
            ...
            raise AxBISyntaxErrorException([error]) from ex
            ...
            raise AxBITemplateException(message) from ex
...
def _get_request_dataset_id_and_form_data() -> tuple[Any | None, dict[str, Any]]:
    # pylint: disable=import-outside-toplevel
    from axbi.views.utils import loads_request_json
...
def get_dataset_id_from_context(metric_key: str) -> int:
    ...
    if isinstance(context_form_data, dict) and context_form_data:
        ...
        if chart_id := (
            ...
            if not chart_data:
                raise AxBITemplateException(exc_message)
