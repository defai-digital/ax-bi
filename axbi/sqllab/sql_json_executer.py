...
class SynchronousSqlJsonExecutor(SqlJsonExecutorBase):
    ...
    def execute(
        ...
        try:
            ...
            raise AxBIGenericDBErrorException(
        ...
        if data.get("status") == QueryStatus.FAILED:  # type: ignore
            ...
            if data["errors"]:  # type: ignore
                raise AxBIErrorsException(
                    [AxBIError(**params) for params in data["errors"]]  # type: ignore
                )
            # old string-only error message
            raise AxBIGenericDBErrorException(data["error"])  # type: ignore
...
class ASynchronousSqlJsonExecutor(SqlJsonExecutorBase):
    def execute(
        ...
        try:
            ...
            error = AxBIError(
                message=message,
                error_type=AxBIErrorType.ASYNC_WORKERS_ERROR,
            ...
            raise AxBIErrorException(error) from ex
