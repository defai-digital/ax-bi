...
from axbi.advanced_data_type.types import AdvancedDataTypeResponse
...
from axbi.db_engine_specs.base import TimestampExpression
...
from axbi.sql.parse import sanitize_clause, SQLScript, SQLStatement
...
from axbi.utils.date_parser import get_past_or_future, normalize_time_delta
from axbi.utils.dates import datetime_to_epoch
...
def validate_adhoc_subquery(
    ...
    :raise AxBISecurityException if sql contains sub-queries or
    ...
    if parsed_statement.has_subquery():
        if not is_feature_enabled("ALLOW_ADHOC_SUBQUERY"):
            raise AxBISecurityException(
                AxBIError(
                    error_type=AxBIErrorType.ADHOC_SUBQUERY_NOT_ALLOWED_ERROR,
...
class AuditMixinNullable(AuditMixin):
    ...
    @declared_attr
    def created_by_fk(self):  # type: ignore[no-untyped-def]  # pylint: disable=arguments-renamed
        return sa.Column(
            sa.Integer,
    ...
    @declared_attr
    def changed_by_fk(self):  # type: ignore[no-untyped-def]  # pylint: disable=arguments-renamed
        return sa.Column(
            sa.Integer,
            sa.ForeignKey("ab_user.id"),
            default=get_user_id,
    ...
    @renders("changed_on")
    def changed_on_dttm(self) -> float | None:
    ...
    def _format_time_humanized(self, timestamp: datetime | None) -> str:
        if not timestamp:
            return ""
...
def _all_soft_delete_subclasses() -> list[type[SoftDeleteMixin]]:
    ...
    the listener fires. AxBI imports models eagerly at app init via
    ``axbi.models``; if that ever changes to lazy import, the
...
def clone_model(
    ...
    keep_relations: list[str] | None = None,
    **kwargs: Any,
) -> Model:
    """
...
class ExploreMixin:  # pylint: disable=too-many-public-methods
    ...
    def get_sqla_row_level_filters(
        self,
        template_processor: BaseTemplateProcessor | None = None,  # pylint: disable=unused-argument
        ...
        if expression:
            ...
            if disallowed_functions or disallowed_tables:
                ...
                if disallowed_functions and parsed.check_functions_present(
                    ...
                    raise AxBIDisallowedSQLFunctionException(disallowed_functions)
                if disallowed_tables and parsed.check_tables_present(disallowed_tables):
                    ...
                    # `axbi.sql_lab._validate_query` so the user-facing
                    ...
                    raise AxBIDisallowedSQLTableException(
    ...
    def _raise_for_disallowed_sql(self, sql: str) -> None:
        ...
        if disallowed_tables and parsed_script.check_tables_present(disallowed_tables):
            ...
            raise AxBIDisallowedSQLTableException(found_tables or disallowed_tables)
    ...
    def query(self, query_obj: QueryObjectDict) -> QueryResult:
        ...
        try:
            ...
            # Re-raise AxBIErrorException (includes OAuth2RedirectError)
            # to bubble up to API layer
            if isinstance(ex, (AxBIErrorException, AxBIErrorsException)):
                raise
            df = pd.DataFrame()
    ...
    def dttm_sql_literal(self, dttm: datetime, col: "TableColumn") -> str:
        ...
        if tf:
            if tf in {"epoch_ms", "epoch_s"}:
                # In general, AxBI works with timezone-naive datetime objects
    ...
    def get_time_filter(  # pylint: disable=too-many-arguments
        ...
        start_dttm: sa.DateTime | None,
        end_dttm: sa.DateTime | None,
    ...
    def validate_expression(
        ...
        from axbi.sql_validators.base import SQLValidationAnnotation
    ...
    def _process_expression_template(
        self, expression: str, tp: BaseTemplateProcessor | None
    ) -> str:
    ...
    def get_sqla_query(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches,too-many-statements  # noqa: C901
        ...
        filter: list[utils.QueryObjectFilterClause] | None = None,
        from_dttm: datetime | None = None,
        ...
        for flt in filter or []:
            ...
            sqla_col: Column | None = None
            adhoc_generic_type: GenericDataType | None = None
            is_metric_filter = (
