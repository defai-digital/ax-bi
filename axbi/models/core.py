...
"""A collection of ORM sqlalchemy models for AxBI"""
...
import sshtunnel
from axbi_core.common.models import Database as CoreDatabase
...
from axbi import db, db_engine_specs, is_feature_enabled
...
from axbi.constants import LRU_CACHE_MAX_SIZE, PASSWORD_MASK
from axbi.databases.utils import make_url_safe
from axbi.db_engine_specs.base import MetricType, TimeGrain
...
from axbi.models.helpers import AuditMixinNullable, ImportExportMixin, UUIDMixin
from axbi.result_set import AxBIResultSet
from axbi.sql.parse import SQLScript, Table
from axbi.utils import cache as cache_util, core as utils, json
from axbi.utils.backports import StrEnum
from axbi.utils.core import get_query_source_from_request, get_username
...
if TYPE_CHECKING:
    from axbi_core.queries.types import AsyncQueryHandle, QueryOptions, QueryResult
...
class Database(CoreDatabase, AuditMixinNullable, ImportExportMixin):  # pylint: disable=too-many-public-methods
    ...
    def function_names(self) -> list[str]:
        try:
            ...
            # more info in: https://github.com/defai-digital/ax-bi/issues/9678
            logger.error(
    ...
    def is_oauth2_enabled(self) -> bool:
        ...
        admins to create custom OAuth2 clients from the AxBI UI, and assign them to
        specific databases.
        """
        try:
    ...
    def execute(
        ...
        """
        from axbi.sql.execution import SQLExecutor
...
class Log(Model):  # pylint: disable=too-few-public-methods
    """ORM object used to log AxBI actions to the database"""
