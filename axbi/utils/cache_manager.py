...
CACHE_IMPORT_PATH = "axbi.extensions.metastore_cache.AxBIMetastoreCache"

# Hash function lookup table matching axbi.utils.hashing
...
class ExploreFormDataCache(AxBICache):
...
class CacheManager:
    def __init__(self) -> None:
        ...
        self._cache = AxBICache()
        self._data_cache = AxBICache()
        self._thumbnail_cache = AxBICache()
        self._filter_state_cache = AxBICache()
    ...
    def _init_cache(
        ...
        if (required and cache_type is None) or cache_type == "AxBIMetastoreCache":
