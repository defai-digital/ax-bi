...
class ReservedUrlParameters(StrEnum):
    """
    Reserved URL parameters that are used internally by AxBI. These will not be
...
def get_user_id() -> int | None:
    ...
    identifier is encoded as a `str` whereas in AxBI all identifiers are encoded as
    an `int`.
...
def is_test() -> bool:
    return parse_boolean_string(os.environ.get("AX_BI_TESTENV") or "false")
...
def check_is_safe_zip(zip_file: ZipFile) -> None:
    """
    Checks whether a ZIP file is safe, raises AxBIException if not.
    ...
    for zip_file_element in zip_file.infolist():
        if zip_file_element.file_size > app.config["ZIPPED_FILE_MAX_SIZE"]:
            raise AxBIException("Found file with size above allowed threshold")
        ...
        if uncompress_size > app.config["ZIP_FILE_MAX_TOTAL_SIZE"]:
            raise AxBIException("Found total uncompressed size above allowed threshold")
    ...
    if (
        ...
        raise AxBIException("Zip compress ratio above allowed threshold")
