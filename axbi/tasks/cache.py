...
from axbi.utils.date_parser import parse_human_datetime
...
def get_dash_url(dashboard: Dashboard) -> str:
    ...
    with current_app.test_request_context():
        baseurl = (
            ...
            or "{AXBI_WEBSERVER_PROTOCOL}://"
            "{AXBI_WEBSERVER_ADDRESS}:"
            "{AXBI_WEBSERVER_PORT}".format(**current_app.config)
        )
...
class Strategy:  # pylint: disable=too-few-public-methods
    ...
    Strategies can be configured in `axbi/config.py`:
...
def cache_warmup(
    ...
    warmup_username = current_app.config.get("AXBI_CACHE_WARMUP_USER")
    if not warmup_username:
        message = (
            "AXBI_CACHE_WARMUP_USER is not configured. Set it to a dedicated "
    ...
    if not user:
        message = (
            ...
            "AXBI_CACHE_WARMUP_USER with a valid username."
        )
        logger.error(message)
        return message
