    ...
    with override_user(_load_user_from_job_metadata(job_metadata), force=False):
        try:
            ...
            if isinstance(ex, AxBIErrorException):
                ...
                # Fallback for non-AxBI exceptions
    ...
    with override_user(_load_user_from_job_metadata(job_metadata), force=False):
        try:
            ...
            if viz_obj.has_error(payload):
                raise AxBIVizException(errors=_get_viz_errors(payload))
            ...
            result_url = f"{AX_BI_ROUTE_PREFIX}/explore_json/data/{cache_key}"
            async_query_manager.update_job(
                job_metadata,
                async_query_manager.STATUS_DONE,
                result_url=result_url,
            ...
            if isinstance(ex, AxBIVizException):
                errors = ex.errors
            # Extract SIP-40 style errors when available
            elif isinstance(ex, AxBIErrorException):
