...
to axbi/examples/ and loaded via the example loading system.

Usage:
    axbi export-example --dashboard-id 123 --name my_example
    axbi export-example --dashboard-slug my-dashboard --name my_example
"""
...
@click.option(
    "--output-dir",
    "-o",
    default="axbi/examples",
    help="Output directory (default: axbi/examples)",
)
@click.option(
...
def export_example(  # noqa: C901
    ...
    Creates a folder structure in axbi/examples/ that can be loaded
    ...
    \b
        ...
        axbi export-example -d 1 -n my_example

    \b
        # Export by slug, limit data to 1000 rows
        axbi export-example -s my-dashboard -n my_example --sample-rows 1000

    \b
        # Export metadata only (no data)
        axbi export-example -d 1 -n my_example --no-export-data
    """
    ...
    if dashboard.json_metadata:
        try:
            meta = axbi_json.loads(dashboard.json_metadata)
    ...
    click.echo("\nTo load this example, ensure the folder is in axbi/examples/")
