...
import axbi.utils.database as database_utils
from axbi.utils.decorators import transaction
from axbi.utils.encrypt import ENCRYPTION_ENGINES, SecretsMigrator
...
def sync_tags() -> None:
    ...
    from axbi.common.tags import add_favorites, add_owners, add_types
...
def update_api_docs() -> None:
    ...
    axbi_dir = os.path.abspath(os.path.dirname(__file__))
    openapi_json = os.path.join(
        axbi_dir, "..", "..", "docs", "static", "resources", "openapi.json"
    )
    api_version = "v1"
    ...
    api_spec = APISpec(
        ...
        servers=[{"url": "http://localhost:31423"}],
    )
