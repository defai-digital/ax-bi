...
def load_engine_specs() -> list[type[BaseEngineSpec]]:
    ...
    for ep in entry_points(group="axbi.db_engine_specs"):
        try:
            engine_spec = ep.load()
        except Exception:  # pylint: disable=broad-except
            logger.warning("Unable to load AxBI DB engine spec: %s", ep.name)
            continue
...
def get_engine_spec(backend: str, driver: str | None = None) -> type[BaseEngineSpec]:
    ...
    drivers to work with AxBI even if they are not listed in the DB engine spec
    drivers.
    """  # noqa: E501
