...
Usage:
    python axbi/db_engine_specs/lint_metadata.py [--json] [--strict]
...
import argparse
import json  # noqa: TID251 - standalone script, don't depend on axbi.utils
import sys
...
def check_pypi_package(package_name: str, timeout: float = 5.0) -> bool:
    ...
    try:
        req = urllib.request.Request(  # noqa: S310
            url, headers={"User-Agent": "axbi-lint/1.0"}
        )
...
def generate_markdown_report(reports: list[MetadataReport]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "```python",
            "from axbi.db_engine_specs.base import (",  # noqa: E501
            ...
            "```",
            "",
            "See `axbi/db_engine_specs/README.md` for full documentation.",
            "",
        ]
    )
