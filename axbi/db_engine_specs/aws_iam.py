...
class AWSIAMAuthMixin:
...

    {
        "aws_iam": {
            "enabled": true,
            "role_arn": "arn:aws:iam::222222222222:role/AxBIDatabaseAccess",
            "external_id": "axbi-prod-12345",  # optional
            "region": "us-east-1",
    ...
    aws_iam_custom_errors: dict[str, tuple[AxBIErrorType, str]] = {
        "AccessDenied": (
            AxBIErrorType.CONNECTION_ACCESS_DENIED_ERROR,
            "Unable to assume IAM role. Verify the role ARN and trust policy "
            "allow access from AxBI's IAM role.",
        ),
        "InvalidIdentityToken": (
            AxBIErrorType.CONNECTION_ACCESS_DENIED_ERROR,
            "Invalid IAM credentials. Ensure AxBI has a valid IAM role "
            "with permissions to assume the target role.",
        ),
        "MalformedPolicyDocument": (
            AxBIErrorType.CONNECTION_MISSING_PARAMETERS_ERROR,
    ...
    def get_iam_credentials(
        ...
        :raises AxBISecurityException: If role assumption fails
        """
        ...
        try:
            ...
            assume_role_kwargs: dict[str, Any] = {
                "RoleArn": role_arn,
                "RoleSessionName": "axbi-iam-session",
    ...
    def generate_rds_auth_token(
        ...
        :raises AxBISecurityException: If token generation fails
        """
        try:
            import boto3
            from botocore.exceptions import ClientError
    ...
    def generate_redshift_credentials(
        ...
        :raises AxBISecurityException: If credential generation fails
        """
        try:
            import boto3
            from botocore.exceptions import ClientError
    ...
    def _apply_iam_authentication(
        ...
        :raises AxBISecurityException: If any step fails
        """
        ...
        if not feature_flag_manager.is_feature_enabled("AWS_DATABASE_IAM_AUTH"):
            raise AxBISecurityException(
                AxBIError(
                    ...
                    "AxBI configuration to enable this feature.",
                    error_type=AxBIErrorType.GENERIC_DB_ENGINE_ERROR,
                    level=ErrorLevel.ERROR,
                )
            )
