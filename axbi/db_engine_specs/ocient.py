...
with contextlib.suppress(ImportError, RuntimeError):  # pyocient may not be installed
    # Ensure pyocient inherits AxBI's logging level
    import geojson
    import pyocient
    from flask import current_app as app
    from shapely import wkt

    axbi_log_level = app.config["LOG_LEVEL"]
    pyocient.logger.setLevel(axbi_log_level)
...
def _linestring_to_geo_json(
    ...
    compatible with the AxBI visualization toolkit (powered
    by Deck.gl).
...
# that cannot be processed natively by AxBI.
#
# AxBI serializes temporal objects using a custom serializer
# defined in axbi/utils/core.py (#json_int_dttm_ser(...)). Other
...
def _find_columns_to_sanitize(cursor: Any) -> list[PlacedSanitizeFunc]:
    """
    Cleans the column value for consumption by AxBI.
...
class OcientEngineSpec(BaseEngineSpec):
    ...
    cte_alias = "cte__"
    # Store mapping of axbi Query id -> Ocient ID
    ...
    custom_errors: dict[Pattern[str], tuple[str, AxBIErrorType, dict[str, Any]]] = {
        CONNECTION_INVALID_USERNAME_REGEX: (
            ...
            AxBIErrorType.CONNECTION_INVALID_USERNAME_ERROR,
            {},
        ),
        CONNECTION_INVALID_PASSWORD_REGEX: (
            ...
            AxBIErrorType.CONNECTION_INVALID_PASSWORD_ERROR,
            {},
        ),
        CONNECTION_UNKNOWN_DATABASE_REGEX: (
            __('Could not connect to database: "%(database)s"'),
            AxBIErrorType.CONNECTION_UNKNOWN_DATABASE_ERROR,
            {},
        ),
        CONNECTION_INVALID_HOSTNAME_REGEX: (
            __('Could not resolve hostname: "%(host)s".'),
            AxBIErrorType.CONNECTION_INVALID_HOSTNAME_ERROR,
            {},
        ),
        CONNECTION_INVALID_PORT_ERROR: (
            __("Port out of range 0-65535"),
            AxBIErrorType.CONNECTION_INVALID_PORT_ERROR,
            {},
        ),
    ...
    def fetch_data(cls, cursor: Any, limit: int | None = None) -> list[tuple[Any, ...]]:
        try:
            ...
            with OcientEngineSpec.query_id_mapping_lock:
                del OcientEngineSpec.query_id_mapping[cursor.axbi_query_id]
            raise
    ...
    def get_cancel_query_id(cls, cursor: Any, query: Query) -> str | None:
        ...
        # If None is returned, AxBI will not call cancel_query
        return "DUMMY_VALUE"
