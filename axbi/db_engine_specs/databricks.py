...
from axbi.utils.core import get_user_agent, QuerySource
...
def monkeypatch_dialect() -> None:
    ...
    fixed version requires SQLAlchemy>=2.0, which is not yet compatible with AxBI.
...
class DatabricksPythonConnectorEngineSpec(DatabricksDynamicBaseEngineSpec):
    ...
    metadata = {
        ...
        "pypi_packages": ["ax-bi[databricks]"],
        "install_instructions": "pip install ax-bi[databricks]",
        "connection_string": (
