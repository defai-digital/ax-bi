...
from axbi.db_engine_specs.cloud_capabilities import get_cloud_connector_capability
...
NICE_TO_HAVE_FEATURES = {
    ...
    "dbapi_exception_mapping": "Maps driver exceptions to AxBI exceptions",
    "custom_errors": "Parses error messages and returns AxBI errors",
...
def diagnose(spec: type[BaseEngineSpec]) -> dict[str, Any]:
    ...
    from axbi.sql_validators.presto_db import PrestoDBSQLValidator
...
def generate_feature_tables() -> str:
    ...
    info = {k: v for k, v in info.items() if v["module"].startswith("axbi")}
    ...
    def extract_db_info(db_info: dict[str, Any], key: str) -> str:
        if key == "limit_method":
            ...
            from axbi.sql.parse import LimitMethod
...
def generate_yaml_docs(output_dir: str | None = None) -> dict[str, dict[str, Any]]:
    ...
    for spec in sorted(load_engine_specs(), key=get_name):
        # Skip non-axbi modules (3rd party)
        if not spec.__module__.startswith("axbi"):
            continue
