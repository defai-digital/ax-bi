...
from axbi import cache_manager, db, is_feature_enabled
...
from axbi.models.sql_types.presto_sql_types import (
    Array,
    Date,
    Interval,
    Map,
...
class PrestoBaseEngineSpec(BaseEngineSpec, metaclass=ABCMeta):
    ...
    def convert_dttm(
        ...
        AxBI only defines time zone naive `datetime` objects, though this method
    ...
    def latest_partition(
        ...
        if not indexes:
            raise AxBITemplateException(
                f"Error getting partition for {table}. "
                "Verify that this table has a partition."
            )

        if len(indexes[0]["column_names"]) < 1:
            raise AxBITemplateException("The table should have one partitioned field")
        ...
        **kwargs: Any,
        ...
        for k in kwargs:
            if k not in part_fields:
                ...
                raise AxBITemplateException(msg)
...
class PrestoEngineSpec(PrestoBaseEngineSpec):
    ...
    metadata = {
        ...
        "install_instructions": 'pip install "ax-bi[presto]"',
    ...
    custom_errors: dict[Pattern[str], tuple[str, AxBIErrorType, dict[str, Any]]] = {
        ...
        SCHEMA_DOES_NOT_EXIST_REGEX: (
            ...
            AxBIErrorType.SCHEMA_DOES_NOT_EXIST_ERROR,
            {},
        ),
    ...
    def get_extra_table_metadata(
        ...
        try:
            ...
            raise AxBIDBAPIProgrammingError(
