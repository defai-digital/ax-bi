...
# under the License.
"""The main config file for AxBI

All configuration in this file can be overridden by providing a axbi_config
in your PYTHONPATH as there is a ``from axbi_config import *``
...
from axbi.advanced_data_type.plugins.internet_port import internet_port
from axbi.advanced_data_type.types import AdvancedDataType
from axbi.axbi_typing import CacheConfig
...
from axbi.stats_logger import DummyStatsLogger
from axbi.tasks.types import ExecutorType
from axbi.themes.types import Theme
from axbi.utils import core as utils
from axbi.utils.encrypt import SQLAlchemyUtilsAdapter
from axbi.utils.log import DBEventLogger
from axbi.utils.logging_configurator import DefaultLoggingConfigurator
from axbi.utils.version import get_dev_env_label
...
# https://github.com/defai-digital/ax-bi/blob/main/axbi/utils/log.py
...
AXBI_SECURITY_VIEW_MENU = True

BASE_DIR = str(files("axbi"))
if "AXBI_HOME" in os.environ:
    DATA_DIR = os.environ["AXBI_HOME"]
else:
    DATA_DIR = os.path.expanduser("~/.axbi")

# ---------------------------------------------------------
# AxBI specific config
# ---------------------------------------------------------
VERSION_INFO_FILE = str(files("axbi") / "static/version_info.json")
PACKAGE_JSON_FILE = str(files("axbi") / "static/assets/package.json")
...
# actually running AxBI, we will have already installed,
...
# AxBIClient HTTP retry configuration
# Controls retry behavior for all HTTP requests made through AxBIClient
# This helps handle transient server errors (like 502 Bad Gateway) automatically
AXBI_CLIENT_RETRY_ATTEMPTS = 3  # Maximum number of retry attempts
AXBI_CLIENT_RETRY_DELAY = 1000  # Initial retry delay in milliseconds
AXBI_CLIENT_RETRY_BACKOFF_MULTIPLIER = 2  # Exponential backoff multiplier
AXBI_CLIENT_RETRY_MAX_DELAY = 10000  # Maximum retry delay cap in milliseconds
AXBI_CLIENT_RETRY_JITTER_MAX = 1000  # Maximum random jitter in milliseconds
# HTTP status codes that should trigger retries (502, 503, 504 gateway errors)
AXBI_CLIENT_RETRY_STATUS_CODES = [502, 503, 504]
...
AXBI_WEBSERVER_TIMEOUT = int(timedelta(minutes=1).total_seconds())
...
AXBI_DASHBOARD_PERIODICAL_REFRESH_LIMIT = 0
AXBI_DASHBOARD_PERIODICAL_REFRESH_WARNING_MESSAGE = None

AXBI_DASHBOARD_POSITION_DATA_LIMIT = 65535
...
# Your App secret key. Make sure you override it on axbi_config.py
# or use the `AX_BI_SECRET_KEY` environment variable.
# Use a strong complex alphanumeric string and use a tool to help you generate
# a sufficiently random sequence, ex: openssl rand -base64 42"
SECRET_KEY = os.environ.get("AX_BI_SECRET_KEY") or CHANGE_ME_SECRET_KEY

# The SQLAlchemy connection string.
SQLALCHEMY_DATABASE_URI = (
    f"""sqlite:///{os.path.join(DATA_DIR, "axbi.db")}?check_same_thread=false"""
)
...
WTF_CSRF_EXEMPT_LIST = [
    "axbi.charts.data.api.data",
    "axbi.dashboards.api.cache_dashboard_screenshot",
    "axbi.views.core.explore_json",
    "axbi.views.core.log",
    "axbi.views.datasource.views.samples",
...
# AxBI allows server-side python stacktraces to be surfaced to the
...
# feature is on by default to make AxBI secure by default, but you should
...
RATELIMIT_ENABLED = os.environ.get("AXBI_ENV") == "production"
...
# The AxBI validator requires a minimum length and rejects common
...
from axbi.security.password_complexity import (  # noqa: E402
...
# axbi_config.py, you must also override THEME_DEFAULT to see the change,
# or set THEME_DEFAULT["token"]["brandLogoUrl"] directly.
APP_ICON = "/static/assets/images/ax-bi-logo-horiz.png"
APP_ICON_DARK = "/static/assets/images/ax-bi-logo-horiz-dark.png"
...
# NOTE: Overriding this in axbi_config.py automatically updates the logo link
...
BABEL_DEFAULT_FOLDER = "axbi/translations"
...
# Feature flags
# ---------------------------------------------------
# Feature flags control optional functionality in AxBI. They can be set in
# axbi_config.py to override the defaults below.
#
...
DEFAULT_FEATURE_FLAGS: dict[str, bool] = {
    ...
    # Enable AxBI extensions for custom functionality without modifying core
    # @lifecycle: development
    ...
    # Serve MCP chart listing from the TypeScript sidecar with Python fallback.
    # @lifecycle: development
    "TS_CHART_LIST_SERVING": False,
    # Serve MCP dataset listing from the TypeScript sidecar with Python fallback.
    # @lifecycle: development
    "TS_DATASET_LIST_SERVING": False,
    # Serve MCP database listing from the TypeScript sidecar with Python fallback.
    # @lifecycle: development
    "TS_DATABASE_LIST_SERVING": False,
    ...
    # @lifecycle: testing
    # @docs: https://github.com/defai-digital/ax-bi/blob/main/docs/admin_docs/configuration/alerts-reports.mdx
    "ALERT_REPORTS": False,
    ...
    # Allows users to add a axbi:// DB that can query across databases.
    # Experimental with potential security/performance risks.
    # See AXBI_META_DB_LIMIT.
    # @lifecycle: testing
    # @docs: https://github.com/defai-digital/ax-bi/tree/main/docs/user_docs_versioned_docs/version-6.1.0/databases/supported
    "ENABLE_AXBI_META_DB": False,
    ...
    # @lifecycle: testing
    # @docs: https://github.com/defai-digital/ax-bi/tree/main/docs/developer_docs/contributing
    ...
    # @lifecycle: testing
    # @docs: https://github.com/defai-digital/ax-bi/tree/main/docs/admin_docs/configuration
    "SSH_TUNNELING": False,
    ...
    # Security note: When enabled, ensure AxBI's IAM role has restricted
    ...
    # Enable caching per user key for AxBI cache
    # @lifecycle: stable
    # @category: runtime_config
    ...
    # @lifecycle: stable
    # @category: runtime_config
    # @docs: https://github.com/defai-digital/ax-bi/blob/main/docs/admin_docs/configuration/networking-settings.mdx
    "DISABLE_EMBEDDED_AXBI_LOGOUT": False,
    ...
    # Enable embedded AxBI functionality
    # @lifecycle: stable
    # @category: runtime_config
    "EMBEDDED_AXBI": False,
    ...
    # @lifecycle: stable
    # @category: runtime_config
    # @docs: https://github.com/defai-digital/ax-bi/blob/main/docs/admin_docs/configuration/cache.mdx
    "THUMBNAILS": False,
    # =================================================================
...
# Feature flags may also be set via 'AXBI_FEATURE_' prefixed environment vars.
DEFAULT_FEATURE_FLAGS.update(
    {
        k[len("AXBI_FEATURE_") :]: utils.parse_boolean_string(v)
        for k, v in os.environ.items()
        if re.search(r"^AXBI_FEATURE_\w+", k)
    }
)
...
# -----------------------------------------------------------------------------
# AxBI supports custom theming through Ant Design's theme structure.
#
# Theme Hierarchy:
...
# It is NOT overridden by ``from axbi_config import *`` (underscore prefix)
...
def sync_theme_logo_href(theme: Theme | None, logo_target_path: str | None) -> None:
    ...
    ``THEME_DEFAULT`` / ``THEME_DARK`` are built above, before ``axbi_config.py``
...
# It authenticates as the single user configured via AXBI_CACHE_WARMUP_USER
...
# `axbi.tasks.types.ExecutorType` for a full list of executor options.
...
AXBI_CACHE_WARMUP_USER: str | None = None
...
# (ax-bi-websocket) reads this key and serves it directly, bypassing Flask
# for cache hits.
FAST_CACHE_ENABLED: bool = False
FAST_CACHE_KEY_PREFIX: str = "fc:"

# Cache for dashboard filter state. `CACHE_TYPE` defaults to `AxBIMetastoreCache`
# that stores the values in the key-value table in the AxBI metastore, as it's
# required for AxBI to operate correctly, but can be replaced by any
# `Flask-Caching` backend.
FILTER_STATE_CACHE_CONFIG: CacheConfig = {
    "CACHE_TYPE": "AxBIMetastoreCache",
...
# Cache for explore form data state. `CACHE_TYPE` defaults to `AxBIMetastoreCache`
# that stores the values in the key-value table in the AxBI metastore, as it's
# required for AxBI to operate correctly, but can be replaced by any
# `Flask-Caching` backend.
...
# time grains in axbi/db_engine_specs/base.py).
# For example: to disable 1 second time grain:
# TIME_GRAIN_DENYLIST = ['PT1S']
TIME_GRAIN_DENYLIST: list[str] = []

# Additional time grains to be supported using similar definitions as in
# axbi/db_engine_specs/base.py.
...
# See axbi/common/query_context_processor.get_aggregated_join_column
#
...
DEFAULT_MODULE_DS_MAP = OrderedDict(
    [
        ("axbi.connectors.sqla.models", ["SqlaTable"]),
    ]
)
...
FILENAME = os.path.join(DATA_DIR, "axbi.log")
ROLLOVER = "midnight"
INTERVAL = 1
BACKUP_COUNT = 30
...
# The limit for the AxBI Meta DB when the feature flag ENABLE_AXBI_META_DB is on
AXBI_META_DB_LIMIT: int | None = 1000
...
# Additional static HTTP headers to be served by your AxBI server. Note
...
# from CSV files. For example, 'companyname-axbi'
...
# Roles that are controlled by the API / AxBI and should not be changed
# by humans.
ROBOT_PERMISSION_ROLES = ["Public", "Gamma", "Alpha", "Admin", "sql_lab"]

CONFIG_PATH_ENV_VAR = "AXBI_CONFIG_PATH"
...
SMTP_USER = "axbi"
SMTP_PORT = 25
SMTP_PASSWORD = "axbi"  # noqa: S105
SMTP_MAIL_FROM = "axbi@axbi.com"
...
# See: axbi.db_engine_specs.base.BaseEngineSpec.validate_database_uri
# Example:
...
MACHINE_AUTH_PROVIDER_CLASS = "axbi.utils.machine_auth.MachineAuthProvider"
...
# blocks indefinitely if the AxBI webserver is unreachable from the worker,
# which leaves the report schedule stuck in the WORKING state. Set to None to
# disable (not recommended).
...
# Slack API token for the axbi reports, either string or callable
...
# Send user to a link where they can read more about AxBI
...
# tokens. See https://github.com/defai-digital/ax-bi/issues/20300 for more
# information. The
...
# specified. If you're running multiple AxBI instances you might want to have a
...
# https://github.com/defai-digital/ax-bi/blob/main/docs/admin_docs/configuration/networking-settings.mdx
...
# Some sqlalchemy connection strings can open AxBI to security risks.
...
# (axbi.connectors.sqla.models.SqlaTable), we call this hook
...
GLOBAL_ASYNC_QUERY_MANAGER_CLASS = (
    "axbi.async_events.async_query_manager.AsyncQueryManager"
)
...
# Audience for the AxBI guest token used in embedded mode.
...
# in axbi/custom_database_errors.py.
# Transform raw database errors into user-friendly messages with optional documentation
try:
    from axbi.custom_database_errors import CUSTOM_DATABASE_ERRORS
except ImportError:
...
if CONFIG_PATH_ENV_VAR in os.environ:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("axbi_config", cfg_path)
        ...
        raise
elif importlib.util.find_spec("axbi_config"):
    try:
        # pylint: disable=import-error,wildcard-import,unused-wildcard-import
        import axbi_config
        from axbi_config import *  # noqa: F403, F401

        click.secho(
            f"Loaded your LOCAL configuration at [{axbi_config.__file__}]",
            fg="cyan",
        )
    except Exception:
        logger.exception("Found but failed to import local axbi_config")
        raise

# Final environment variable processing - must be at the very end
# to override any config file assignments
ENV_VAR_KEYS = {
    "AXBI__SQLALCHEMY_DATABASE_URI",
    "AXBI__SQLALCHEMY_EXAMPLES_URI",
}

for env_var in ENV_VAR_KEYS:
    if env_var in os.environ:
        config_var = env_var.replace("AXBI__", "")
