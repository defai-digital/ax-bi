...
from axbi import db, event_logger, security_manager
...
from axbi.connectors.sqla.utils import get_physical_table_metadata
    ...
    def external_metadata_by_name(self, **kwargs: Any) -> FlaskResponse:
        ...
        try:
            if datasource is not None:
                # Get columns from AxBI metadata
                security_manager.raise_for_access(datasource=datasource)
                external_metadata = datasource.external_metadata()
            else:
...
class DatasetEditor(BaseAxBIView):
    route_base = "/dataset"
