...
from axbi.commands.chart.exceptions import ChartNotFoundError
from axbi.commands.chart.warm_up_cache import ChartWarmUpCacheCommand
from axbi.commands.dashboard.exceptions import DashboardAccessDeniedError
from axbi.commands.dashboard.permalink.get import GetDashboardPermalinkCommand
...
from axbi.constants import AX_BI_ROUTE_PREFIX
from axbi.daos.chart import ChartDAO
from axbi.daos.datasource import DatasourceDAO
from axbi.dashboards.permalink.exceptions import DashboardPermalinkGetFailedError
...
from axbi.extensions import async_query_manager, cache_manager
...
from axbi.utils.cache import etag_cache
...
class AxBI(BaseAxBIView):
    """The base views for AxBI!"""
    ...
    def _generate_xlsx(viz_obj: BaseViz) -> FlaskResponse:
        ...
        from axbi.utils.excel import apply_column_types, df_to_excel
    ...
    def explore_json(  # noqa: C901
        ...
        if response_type in (
            ...
            if is_feature_enabled("GRANULAR_EXPORT_CONTROLS"):
                can_export = security_manager.can_access("can_export_data", "AxBI")
            else:
                can_export = security_manager.can_access("can_csv", "AxBI")
            if not can_export:
    ...
    def explore(  # noqa: C901
        ...
        if request.method == "GET":
            return relative_redirect(AxBI.get_redirect_url())
        ...
        if is_feature_enabled("GRANULAR_EXPORT_CONTROLS"):
            slice_download_perm = security_manager.can_access("can_export_data", "AxBI")
        else:
            slice_download_perm = security_manager.can_access("can_csv", "AxBI")
        ...
        except (AxBIException, SQLAlchemyError):
    ...
    def dashboard_permalink(
        ...
        url = url_for(
            "AxBI.dashboard", dashboard_id_or_slug=dashboard_id, permalink_key=key
        )
