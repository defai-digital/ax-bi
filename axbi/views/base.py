...
from axbi.connectors.sqla import models
...
from axbi.models.core import Theme as ThemeModel
from axbi.reports.models import ReportRecipientType
from axbi.themes.types import Theme, ThemeMode
...
from axbi.utils.version import get_version_metadata
from axbi.views.error_handling import json_error_response

from .utils import bootstrap_user_data, get_config_value

FRONTEND_CONF_KEYS = (
    "AXBI_WEBSERVER_TIMEOUT",
    "AXBI_DASHBOARD_POSITION_DATA_LIMIT",
    ...
    "AXBI_CLIENT_RETRY_ATTEMPTS",
    "AXBI_CLIENT_RETRY_DELAY",
    "AXBI_CLIENT_RETRY_BACKOFF_MULTIPLIER",
    "AXBI_CLIENT_RETRY_MAX_DELAY",
    "AXBI_CLIENT_RETRY_JITTER_MAX",
    "AXBI_CLIENT_RETRY_STATUS_CODES",
...
def deprecated(
    ...
    """
    A decorator to set an API endpoint from AxBIView has deprecated.
    Issues a log warning
    """

    def _deprecated(f: Callable[..., FlaskResponse]) -> Callable[..., FlaskResponse]:
        def wraps(self: BaseAxBIView, *args: Any, **kwargs: Any) -> FlaskResponse:
            message = (
                "%s.%s "
...
def get_environment_tag() -> dict[str, Any]:
    ...
    # default is AXBI_ENV (from FLASK_ENV in previous versions)
...
def menu_data(user: User) -> dict[str, Any]:
    ...
    return {
        ...
        "brand": {
            "path": app.config["LOGO_TARGET_PATH"] or url_for("AxBI.welcome"),
        ...
        "navbar_right": {
            ...
            "show_watermark": ("axbi-logo-horiz" not in appbuilder.app_icon),
...
def get_spa_template_context(
    ...
    **template_kwargs: Any,
    ...
    # Priority: theme brandAppName > APP_NAME config > "AX BI" default
    app_name_from_config = app.config.get("APP_NAME", "AX BI")
    for theme_config in [default_theme, dark_theme]:
        ...
        if (
            ...
            or theme_tokens.get("brandAppName") == "AX BI"
        ):
            # If brandAppName not set or is default, check if APP_NAME customized
            if app_name_from_config != "AX BI":
    ...
    default_title = theme_tokens.get("brandAppName", "AX BI")
