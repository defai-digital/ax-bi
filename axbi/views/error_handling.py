...
from axbi.commands.exceptions import CommandException, CommandInvalidError
...
def json_error_response(
    error_details: str | AxBIError | list[AxBIError] | None = None,
    status: int = 500,
    ...
    elif isinstance(error_details, AxBIError):
...
def handle_api_exception(
    ...
    """
    A decorator to catch axbi exceptions. Use it after the @api decorator above
    so axbi exception handler is triggered before the handler for generic
    exceptions.
    """
    ...
    def wraps(self: BaseAxBIView, *args: Any, **kwargs: Any) -> FlaskResponse:
        try:
            ...
            logger.warning("AxBISecurityException", exc_info=True)
        ...
        except AxBIErrorException as ex:
...
def set_app_error_handlers(app: Flask) -> None:  # noqa: C901
    ...
    @app.errorhandler(AxBIErrorException)
    def show_axbi_error(ex: AxBIErrorException) -> FlaskResponse:
        logger.warning("AxBIErrorException", exc_info=True)
        return json_error_response([ex.error], status=ex.status)

    @app.errorhandler(AxBIErrorsException)
    def show_axbi_errors(ex: AxBIErrorsException) -> FlaskResponse:
        logger.warning("AxBIErrorsException", exc_info=True)
        return json_error_response(ex.errors, status=ex.status)
    ...
    def show_http_exception(ex: HTTPException) -> FlaskResponse:
        ...
        if (
            ...
            path = files("axbi") / f"static/assets/{ex.code}.html"
    ...
    def show_command_errors(ex: CommandException) -> FlaskResponse:
        ...
        CommandException it should be fixed to map it to AxBIErrorException
        or AxBIErrorsException, with a specific status code and error type
        """
        logger.warning("CommandException", exc_info=True)

        if "text/html" in request.accept_mimetypes and not app.config["DEBUG"]:
            path = files("axbi") / "static/assets/500.html"
            # Try to serve HTML file; fall back to JSON if not built
            try:
