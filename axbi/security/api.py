...
from axbi.commands.exceptions import ForbiddenError
from axbi.exceptions import AxBIGenericErrorException
from axbi.extensions import db, event_logger
...
class SecurityRestApi(BaseAxBIApi):
    ...
    def guest_token(self) -> Response:
        """Get a guest token that can be used for auth in embedded AxBI.
        ---
        post:
        ...
        try:
            ...
            if guest_token_validator_hook is not None:
                if callable(guest_token_validator_hook):
                    ...
                    raise AxBIGenericErrorException(
...
class UserRegistrationsRestAPI(BaseAxBIModelRestApi):
    """
