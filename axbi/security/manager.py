...
from axbi.constants import RouteMethod
...
if TYPE_CHECKING:
    ...
    from axbi.explorables.base import Explorable
...
class AxBIPermissionViewMenuApi(PermissionViewMenuApi):
    """
...
class AxBISecurityManager(  # pylint: disable=too-many-public-methods
    SecurityManager
):
    userstatschartview = None
    register_axbi_auth_view = True
    """Set to False in subclasses that provide their own auth view."""
    register_axbi_registeruser_view = True
    """Set to False in subclasses that provide their own register user view."""
    READ_ONLY_MODEL_VIEWS = {"Database", "DynamicPlugin"}

    role_api = AxBIRoleApi
    user_api = AxBIUserApi
    group_api = AxBIGroupApi
    permission_view_menu_api = AxBIPermissionViewMenuApi
    ...
    def create_api_key(
        ...
        display_hint = f"{key_body[:4]}{raw_key[-5:]}"
    ...
    SQLLAB_ONLY_PERMISSIONS = {
        ...
        ("can_sqllab", "AxBI"),
        ("can_test_conn", "AxBI"),  # Deprecated permission remove on 3.0.0
    ...
    SQLLAB_EXTRA_PERMISSION_VIEWS = {
        ("can_csv", "AxBI"),  # Deprecated permission remove on 3.0.0
        ("can_export_data", "AxBI"),
    ...
    PUBLIC_ROLE_PERMISSIONS = {
        ...
        ("can_dashboard", "AxBI"),
        ("can_slice", "AxBI"),
        ("can_explore_json", "AxBI"),
        ("can_dashboard_permalink", "AxBI"),
    ...
    def on_user_login(self, user: Any) -> None:
        # pylint: disable=import-outside-toplevel
        from axbi.security.session_invalidation import stamp_login_time
    ...
    def request_loader(self, request: Request) -> User | None:
        ...
        if feature_flag_manager.is_feature_enabled("EMBEDDED_AXBI"):
    ...
    def get_catalog_perm(
        ...
        :param database: The AxBI database or database name
    ...
    def can_drill_dataset_via_dashboard_access(
        ...
        return bool(
            (
                (
                    is_feature_enabled("EMBEDDED_AXBI")
    ...
    def get_dashboard_access_error_object(  # pylint: disable=invalid-name
        ...
        """
        Return the error object for the denied AxBI dashboard.

        :param dashboard: The denied AxBI dashboard
    ...
    def get_datasource_access_error_msg(
        ...
        """
        Return the error message for the denied AxBI datasource.

        :param datasource: The denied AxBI datasource
    ...
    def get_datasource_access_link(  # pylint: disable=unused-argument
        ...
        """
        Return the link for the denied AxBI datasource.
    ...
    def get_datasource_access_error_object(  # pylint: disable=invalid-name
        ...
        """
        Return the error object for the denied AxBI datasource.
    ...
    def get_table_access_error_object(self, tables: set["Table"]) -> AxBIError:
        """
    ...
    def create_custom_permissions(self) -> None:
        ...
        self.add_permission_view_menu("can_csv", "AxBI")
        self.add_permission_view_menu("can_export_data", "AxBI")
        self.add_permission_view_menu("can_export_image", "AxBI")
        self.add_permission_view_menu("can_copy_clipboard", "AxBI")
        self.add_permission_view_menu("can_share_dashboard", "AxBI")
        self.add_permission_view_menu("can_share_chart", "AxBI")
        self.add_permission_view_menu("can_sqllab", "AxBI")
    ...
    def _create_missing_semantic_perms(
        ...
        from axbi.semantic_layers.models import (  # pylint: disable=import-outside-toplevel
            SemanticLayer,
            SemanticView,
        )
    ...
    def sync_role_definitions(self) -> None:
        """
        Initialize the AxBI application with security roles and such.
        """
    ...
    def _update_vm_datasources_access(  # pylint: disable=too-many-locals
        ...
        :return: A list of changed view menus (permission resource names)
        """  # noqa: E501
        from axbi.connectors.sqla.models import (  # pylint: disable=import-outside-toplevel
            SqlaTable,
        )
        from axbi.models.slice import (  # pylint: disable=import-outside-toplevel
            Slice,
        )
    ...
    def dataset_after_insert(
        ...
        :return:
        """
        from axbi.models.core import (  # pylint: disable=import-outside-toplevel
            Database,
        )
    ...
    def raise_for_access(  # noqa: C901
        ...
        :param database: The AxBI database
        :param datasource: The AxBI datasource
        :param query: The SQL Lab query
        :param query_context: The query context
        :param table: The AxBI table (requires database)
        ...
        :param force_dataset_match: When True, the historical
            ...
            table must resolve to a registered AxBI dataset the user
        ...
        if database and table or query:
            ...
            # registered AxBI dataset the user can access. Other callers
            ...
            if denied:
                raise AxBISecurityException(self.get_table_access_error_object(denied))
            ...
            if not (
                ...
                or (
                    ...
                    and (
                        ...
                        or (
                            is_feature_enabled("EMBEDDED_AXBI") and self.is_guest_user()
                        )
                    )
                    and (
                        (
        ...
        if chart:
            ...
            raise AxBISecurityException(self.get_chart_access_error_object(chart))
    ...
    def validate_guest_token_resources(resources: GuestTokenResources) -> None:
        # pylint: disable=import-outside-toplevel
        from axbi.commands.dashboard.embedded.exceptions import (
    ...
    def is_guest_user(user: Any | None = None) -> bool:
        ...
        if not is_feature_enabled("EMBEDDED_AXBI"):
            return False
    ...
    def raise_for_ownership(self, resource: Model) -> None:
        ...
        :raises AxBISecurityException: If the current user is not an owner
        """
        # Inline import: ``axbi.models.helpers`` transitively imports
        # ``axbi.models.core``, which depends on lazily-initialised
        # ``axbi.feature_flag_manager``. A top-level import here would
        # create a circular dependency (security ↔ models.core ↔ axbi).
        from axbi.models.helpers import (  # pylint: disable=import-outside-toplevel  # noqa: E501
    ...
    def register_views(self) -> None:
        from axbi.views.auth import AxBIAuthView, AxBIRegisterUserView

        if self.register_axbi_auth_view:
            self.auth_view = self.appbuilder.add_view_no_menu(AxBIAuthView)
        if self.register_axbi_registeruser_view:
            self.registeruser_view = self.appbuilder.add_view_no_menu(
                AxBIRegisterUserView
            )
