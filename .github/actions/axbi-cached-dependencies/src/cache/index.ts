...
import { getInput, maybeArrayToString } from '../utils/inputs';
...
export const actions = {
  restore(inputs: Inputs) {
    const paths = inputs[InputName.Path]?.split(/\r?\n/).filter(Boolean) || [];
    const restoreKeys = inputs[InputName.RestoreKeys]
      ?.split(/\r?\n/)
      .filter(Boolean);
    return restoreCache(
      paths,
      inputs[InputName.Key] || '',
      restoreKeys?.length ? restoreKeys : undefined,
    );
  },
  save(inputs: Inputs) {
    const paths = inputs[InputName.Path]?.split(/\r?\n/).filter(Boolean) || [];
    return saveCache(paths, inputs[InputName.Key] || '');
  },
};
