...
@pytest.mark.unit
def test_optional_file_exists_rejects_changed_content(
    isolated_filesystem,
    monkeypatch,
):
    """Test optional file helper refuses content changes during validation."""
    metadata_file = isolated_filesystem / "extension.json"
    metadata_file.write_text("{}")
    original_get_read_path_identity = cli.get_read_path_identity
    identity_reads = 0

    def update_file_after_first_identity(path):
        nonlocal identity_reads
        identity = original_get_read_path_identity(path)
        if path == metadata_file:
            identity_reads += 1
            if identity_reads == 1:
                metadata_file.write_text('{"version": "9.9.9"}')
        return identity

    monkeypatch.setattr(cli, "get_read_path_identity", update_file_after_first_identity)
...
def test_input_file_exists_rejects_changed_content(
    ...
    """Test input file helper refuses content changes during validation."""
    metadata_file = isolated_filesystem / "extension.json"
    metadata_file.write_text("{}")
    original_get_read_path_identity = cli.get_read_path_identity
    identity_reads = 0
...
def test_metadata_loaders_fail_when_read_boundary_changes_after_validation(
    ...
    monkeypatch.setattr(f"axbi_extensions_cli.cli.{reader_name}", lambda _: None)
...
def test_validate_command_success(cli_runner, isolated_filesystem):
    ...
    with patch("axbi_extensions_cli.cli.validate_npm") as mock_validate:
...
def test_validate_fails_with_invalid_extension_json(
    ...
    with patch("axbi_extensions_cli.cli.validate_npm"):
        result = cli_runner.invoke(app, ["validate"])
...
def test_validate_fails_on_version_mismatch(
    ...
    assert "ax-bi-extensions update" in result.output
