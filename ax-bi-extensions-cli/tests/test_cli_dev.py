...
from axbi_extensions_cli.cli import FrontendChangeHandler, app


# Dev Command Tests
@pytest.mark.cli
@patch("axbi_extensions_cli.cli.Observer")
@patch("axbi_extensions_cli.cli.validate_npm")
@patch("axbi_extensions_cli.cli.init_frontend_deps")
@patch("axbi_extensions_cli.cli.rebuild_frontend")
@patch("axbi_extensions_cli.cli.rebuild_backend")
@patch("axbi_extensions_cli.cli.build_manifest")
@patch("axbi_extensions_cli.cli.write_manifest")
...
def test_dev_command_initial_build(
    ...
    with patch("axbi_extensions_cli.cli.Observer") as mock_observer_class:
        mock_observer = Mock()
        mock_observer_class.return_value = mock_observer
...
def test_frontend_watcher_function_coverage(isolated_filesystem):
    ...
    with (
        patch("axbi_extensions_cli.cli.rebuild_frontend") as mock_rebuild,
        patch("axbi_extensions_cli.cli.build_manifest") as mock_build,
        patch("axbi_extensions_cli.cli.write_manifest") as mock_write,
    ):
...
def test_backend_watcher_function_coverage(isolated_filesystem):
    ...
    with patch("axbi_extensions_cli.cli.rebuild_backend") as mock_rebuild:
