...
import click
import pytest
from axbi_core.extensions.types import Manifest

import axbi_extensions_cli.cli as cli
from axbi_extensions_cli.cli import (
    app,
    build_manifest,
    clean_dist,
...
# Build Command Tests
@pytest.mark.cli
@patch("axbi_extensions_cli.cli.validate_npm")
@patch("axbi_extensions_cli.cli.init_frontend_deps")
@patch("axbi_extensions_cli.cli.run_frontend_build")
@patch("axbi_extensions_cli.cli.copy_frontend_dist")
@patch("axbi_extensions_cli.cli.rebuild_backend")
@patch("axbi_extensions_cli.cli.read_toml")
...
def test_build_command_reports_failed_dist_cleanup_during_rollback(
    ...
    from axbi_extensions_cli import cli
...
"""
    ...
    monkeypatch.setattr(
        "axbi_extensions_cli.cli.start_dist_replacement",
...
def test_start_dist_replacement_restores_dist_when_replacement_create_fails(
    ...
    monkeypatch.setattr(
        "axbi_extensions_cli.cli.ensure_output_directory",
...
def test_copy_output_file_rejects_source_changed_during_copy(
    ...
    monkeypatch.setattr("axbi_extensions_cli.cli.shutil.copy2", swap_source_after_copy)
...
def test_copy_output_file_rejects_target_changed_during_copy(
    ...
    monkeypatch.setattr("axbi_extensions_cli.cli.shutil.copy2", swap_target_after_copy)
...
def test_copy_output_file_rejects_target_parent_changed_during_copy(
    ...
    monkeypatch.setattr(
        "axbi_extensions_cli.cli.shutil.copy2",
...
def test_copy_output_file_rejects_target_content_changed_during_copy(
    ...
    monkeypatch.setattr(
        "axbi_extensions_cli.cli.shutil.copy2", change_target_after_copy
    )
...
def test_init_frontend_deps_rejects_changed_frontend_before_install(
    ...
    monkeypatch.setattr(
        "axbi_extensions_cli.cli.validate_npm",
...
def test_clean_dist_frontend_removes_frontend_dist(isolated_filesystem):
    ...
    from axbi_extensions_cli.cli import clean_dist_frontend
...
def test_run_frontend_build_with_output_messages(isolated_filesystem):
    ...
    from axbi_extensions_cli.cli import run_frontend_build
    ...
    with (
        ...
        patch("axbi_extensions_cli.cli.validate_npm", return_value="npm"),
    ):
...
def test_rebuild_frontend_handles_build_results(
    ...
    from axbi_extensions_cli.cli import rebuild_frontend
    ...
    with patch("axbi_extensions_cli.cli.run_frontend_build") as mock_build:
...
def test_rebuild_backend_calls_copy_and_shows_message(isolated_filesystem):
    ...
    from axbi_extensions_cli.cli import rebuild_backend
    ...
    with patch("axbi_extensions_cli.cli.copy_backend_files") as mock_copy:
...
"""
    ...
    monkeypatch.setattr("axbi_extensions_cli.cli.shutil.copy2", track_copy)
