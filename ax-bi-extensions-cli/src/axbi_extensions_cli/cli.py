...
from axbi_extensions_cli.constants import MIN_NPM_VERSION
from axbi_extensions_cli.exceptions import ExtensionNameError
from axbi_extensions_cli.types import ExtensionNames
from axbi_extensions_cli.utils import (
    NodeIdentity,
    PathIdentity,
...
def start_dist_replacement(
    ...
    if not dist_dir.exists():
        ...
        return None, None, None, None, None, replacement_identity
    ...
    backup_root_parent_identity = get_read_parent_identity(backup_root)
    if backup_root_parent_identity is None:
        raise click.ClickException(
...
def copy_output_file(source: Path, target: Path, label: str) -> None:
    ...
    target_parent_identity = get_read_parent_identity(target)
    if target_parent_identity is None:
        raise click.ClickException(
            f"Refusing to copy {label}: target parent path is unsafe."
        )
    try:
        current_parent_identity = get_read_parent_identity(target)
        if (
...
def require_optional_directory(path: Path, label: str) -> None:
    ...
    if path.exists():
        ...
        if (
            get_read_parent_identity(path) != parent_identity
            or current_identity != directory_identity
        ):
...
def optional_file_exists(path: Path, label: str) -> bool:
    ...
    if (
        get_read_parent_identity(path) != parent_identity
        or current_identity != file_identity
    ):
        raise click.ClickException(f"{label} path changed during validation.")
    return True
...
def get_backend_build_patterns(
    ...
    extension_config = tool_config.get("axbi_extensions", {})
    if not isinstance(extension_config, dict):
        raise click.ClickException(
            "Invalid backend build config: [tool.axbi_extensions] must be a table."
        )
    ...
    if not isinstance(build_config, dict):
        raise click.ClickException(
            ...
            "[tool.axbi_extensions.build] must be a table."
        )
...
def validate() -> None:
    ...
    if mismatches:
        ...
        click.secho(
            "Run `ax-bi-extensions update` to sync from extension.json.",
            fg="yellow",
        )
        sys.exit(1)
...
def build(ctx: click.Context) -> None:
    ...
    (
        ...
        dist_backup_root_parent_identity,
