 ...
 *
 * @param {string[]} candidateFiles paths relative to `ax-office-frontend/`
 */
...
function normalizeCliFile(file) {
  return file.replace(/^ax-office-frontend\//, '');
}
...
function main() {
  ...
  if (files.length === 0) {
    ...
    // Filter to only JS/TS files and remove ax-office-frontend prefix
    files = files
