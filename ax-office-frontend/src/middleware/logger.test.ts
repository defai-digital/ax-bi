...
describe('logger middleware', () => {
  ...
  test('should POST an event to /ax-office/log/ when called', () => {
    (logger as Function)(mockStore)(next)(action);
    expect(next.mock.calls.length).toBe(0);

    jest.advanceTimersByTime(2000);
    expect(postStub.mock.calls.length).toBe(1);
    expect(postStub.mock.calls[0][0].endpoint).toMatch('/ax-office/log/');
  });
  ...
  test('should use navigator.sendBeacon if it exists', () => {
    ...
    expect(endpoint).toMatch('/ax-office/log/');
  });
