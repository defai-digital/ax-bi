...
```bash
ax-bi import_datasources -p <path or filename>
```
...
```bash
ax-bi import_datasources -p <path> -r
```
...
```bash
ax-bi import_datasources -p <path / filename> -s columns,metrics
```
