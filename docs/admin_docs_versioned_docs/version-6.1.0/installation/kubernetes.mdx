...
Just like any typical Helm chart, you'll need to craft a `values.yaml` file that would define/override any of the values exposed into the default [values.yaml](https://github.com/defai-digital/ax-bi/tree/main/helm/superset/values.yaml), or from any of the dependent charts it depends on:
...
ax-bi-init-db-zlm9z                  0/1     Completed   0          111s
superset-postgresql-0                   1/1     Running     0          6d20h
superset-redis-master-0                 1/1     Running     0          6d20h
ax-bi-worker-75b48bbcc-jmmjr         1/1     Running     0          4m8s
ax-bi-worker-75b48bbcc-qrq49         1/1     Running     0          4m12s
```

The exact list will depend on some of your specific configuration overrides but you should generally expect:

- N `superset-xxxx-yyyy` and `ax-bi-worker-xxxx-yyyy` pods (depending on your `supersetNode.replicaCount` and `supersetWorker.replicaCount` values)
...
[pyproject.toml](https://github.com/defai-digital/ax-bi/blob/main/pyproject.toml)
