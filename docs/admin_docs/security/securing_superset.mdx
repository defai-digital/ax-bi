...
### **`AX_BI_SECRET_KEY` Management (CRITICAL)**
...
#### **Initial Setup:**
  ...
  - [ ] A unique, strong `AX_BI_SECRET_KEY` is generated and secured in an environment variable or secrets vault.
...
#### **Ongoing Maintenance:**
  ...
  - [ ] Rotate the `AX_BI_SECRET_KEY` periodically (e.g., quarterly) and after any potential security incident.
...
`AX_BI_SECRET_KEY` is not the only security-critical secret in a Superset deployment. Maintain an inventory of all such secrets, store each in a secrets manager (not in `superset_config.py` or version control), assign an owner, and rotate them on a defined cadence as well as after any suspected compromise.

| Secret | Purpose | Risk if leaked | Suggested rotation |
|---|---|---|---|
| `AX_BI_SECRET_KEY` | Signs session cookies; key material for encrypting stored DB credentials (Fernet/AES) | Forged sessions (auth bypass / privilege escalation); decryption of exfiltrated metadata-DB secrets | Quarterly + post-incident |
...
- After a suspected compromise, rotate **all** of the above, not only `AX_BI_SECRET_KEY`.
