...
When an authenticated user's navbar first loads, AX BI automatically creates a dedicated `AX BI MCP` key if none exists. The top-right navigation shows the username and a masked hint such as `M8hay********hfdsG`. The hint contains five leading and five trailing characters from the key; it is not a usable credential.
