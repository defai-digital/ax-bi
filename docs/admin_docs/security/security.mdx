...
When an authenticated user's navbar first loads, AX BI automatically creates a dedicated `AX BI MCP` key if none exists. The top-right navigation shows the username and a masked hint such as `sst_e5a7********khTpzMxY`. The hint contains eight leading and eight trailing characters from the full key; it is not a usable credential.
