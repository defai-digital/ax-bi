...
After an authenticated user's first login, AX BI automatically creates a dedicated `AX BI MCP` key. The top-right navigation shows the username and a masked hint such as `M7yh-**********-9yGaH`.

Click the eye button beside the hint to rotate and copy the key. Every click creates a new key and revokes the previous dedicated MCP key, so any MCP client configured with the old key must be updated.

Admin, Alpha, and Gamma users receive the self-service API-key permissions by default. The endpoint always creates, lists, and revokes keys for the authenticated user only. Public users cannot manage keys.
...
The MCP request is executed with the current roles, object ACLs, and row-level security rules of the user who created the key.
