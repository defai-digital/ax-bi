...
the **ax-bi init** command (often done between Superset versions).

The built-in role descriptions below summarize the intended access model.
...
permissions are synced to the Public role when you run `ax-bi init`:
...
- **Role synchronization:** Built-in role permissions (Admin, Alpha, Gamma, sql_lab, and Public
  when `PUBLIC_ROLE_LIKE = "Public"`) are synchronized when you run `ax-bi init`. Any manual
...
AX BI supports user-bound API keys for MCP clients and other programmatic integrations.

#### Enabling API Key Authentication

API key authentication and the matching management UI are enabled by default. Operators can disable both when an external JWT/OIDC provider is the only supported authentication method:
...
The FAB setting registers the API-key endpoints, the MCP setting controls whether MCP accepts those keys, and the feature flag controls the management UI.

#### Creating an API Key

When an authenticated user's navbar first loads, AX BI automatically creates a dedicated `AX BI MCP` key if none exists. The top-right navigation shows the username and a masked hint such as `M8hayd7-**********-iay8hfdsG`. The hint contains seven leading and nine trailing characters from the random key body; it is not a usable credential.

Click the eye button beside the hint to rotate and copy the key. Every click creates a new key, copies its one-time plaintext, and only then revokes the previous dedicated MCP key. If copying fails, AX BI revokes the replacement and keeps the previous key active. Any MCP client configured with the old key must be updated after a successful rotation.

Admin, Alpha, and Gamma users receive the self-service API-key permissions after `ax-bi init`. The endpoint always creates, lists, and revokes keys for the authenticated user only. Public users cannot manage keys.
...
The REST or MCP request is executed with the current roles, object ACLs, and row-level security rules of the user who created the key. Revoking a key takes effect without changing the user's other sessions or keys.
...
:::caution
Store plaintext API keys in a secret manager, never source control or logs. Anyone with a valid key can make requests on behalf of the creating user. Revoke keys promptly if they are compromised by deleting them from the **API Keys** section of your User Info page or by rotating the navbar key.
:::
