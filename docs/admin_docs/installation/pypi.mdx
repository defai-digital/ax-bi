...
Refer to the
[pyproject.toml](https://github.com/defai-digital/ax-bi/blob/main/pyproject.toml) file for the list of
...
[pyproject.toml](https://github.com/defai-digital/ax-bi/blob/main/pyproject.toml) file for a list of Python
...
You can create and activate a virtual environment using the following commands. Ensure you are using a compatible version of python. You might have to explicitly use for example `python3.12` instead of `python3`.
...
```bash
export AX_BI_SECRET_KEY=YOUR-SECRET-KEY # For production use, make sure this is a strong key, for example generated using `openssl rand -base64 42`. See https://superset.apache.org/admin-docs/configuration/configuring-superset#specifying-a-secret_key
...
ax-bi run -p 8088 --with-threads --reload
