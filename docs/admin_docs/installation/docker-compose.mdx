...
:::tip
By default, we mount the local ax-bi-frontend folder here and run `npm install` as well
...
the `ax-bi-frontend/` folder will nuke the various' packages `node_module/` folders
...
mentioned in [docker/README.md](https://github.com/defai-digital/ax-bi/tree/main/docker#configuration)
...
One important variable is `SUPERSET_LOAD_EXAMPLES` which determines whether the `ax-bi-init` service will populate example data and visualizations into the metadata database. These examples
