...
AX BI includes a built-in [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that lets AI assistants -- Claude, ChatGPT, and other MCP-compatible clients -- interact with your AX BI instance. Through MCP, clients can list dashboards, query datasets, execute SQL, create charts, and more.
...
```mermaid
flowchart LR
    A["AI Client<br/>(Claude, ChatGPT, etc.)"] -- "MCP protocol<br/>(HTTP + JSON-RPC)" --> B["MCP Server<br/>(:31421/mcp)"]
    B -- "AX BI context<br/>(app, db, RBAC)" --> C["AX BI<br/>(:31423)"]
...
```bash
ax-bi mcp run --host 127.0.0.1 --port 31421
```

| Flag | Default | Description |
|------|---------|-------------|
| `--host` | `127.0.0.1` | Host to bind to |
| `--port` | `31421` | Port to bind to |
...
Sign in to AX BI, then click the eye action beside the masked MCP key in the
top-right navbar. Store the copied key in the client platform's secret store.

The eye action always generates a replacement, so update any client using the
previous value.
...
flowchart TD
    ...
    F -- No --> K{"User API key<br/>prefix?"}
    K -- Yes --> FAB["Validate salted hash<br/>and load key owner"]
    K -- No --> AE{"MCP_AUTH_ENABLED?"}
...
### User-bound API keys (default)

AX BI enables user-bound API-key authentication by default. When an
authenticated navbar loads, it creates a key named `AX BI MCP` if the user does
not already have one. The top-right control displays only a hint such as
`M8hayd7-**********-iay8hfdsG`.

Click the eye action to create and copy a replacement. AX BI revokes the old
managed key only after the new plaintext is copied successfully. The full
`sst_...` value is returned only at creation and is stored server-side only as a
salted hash.
...
The key resolves to its AX BI owner on every request. The owner's current FAB
roles, datasource permissions, object ACLs, and row-level security rules remain
in force; a key does not grant additional access. Public users cannot manage
keys, and the list/get/revoke endpoints are restricted to the authenticated
owner.

Run `ax-bi init` after installation or upgrade to synchronize the built-in
Admin, Alpha, and Gamma `ApiKey` permissions. Operators using only an external
JWT/OIDC provider can disable the feature:
...
For deployments with a centralized identity provider, enable JWT-based authentication. The MCP server validates a Bearer token on every request, and JWT may coexist with user-bound API keys.
...
**With a user-bound API key or JWT:**
  ...
  "mcpServers": {
    "superset": {
      ...
      "args": [
        "-y",
        "mcp-remote@latest",
        "http://your-ax-bi-host:31421/mcp",
        "--header",
        "Authorization: Bearer YOUR_MCP_API_KEY"
      ]
    }
...
```bash
curl -X POST http://localhost:31421/mcp \
...
ax-bi run -h 0.0.0.0 -p 8088

# Terminal 2 -- MCP server
ax-bi mcp run --host 0.0.0.0 --port 31421
```
...
server {
    ...
    # MCP endpoint
    location /mcp {
        proxy_pass http://127.0.0.1:31421/mcp;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
...
services:
  ...
  mcp:
    image: apache/superset:latest
    command: ["ax-bi", "mcp", "run", "--host", "0.0.0.0", "--port", "31421"]
    ports:
      - "31421:31421"
    volumes:
      - ./superset_config.py:/app/superset_config.py
...
| `MCP_SERVICE_PORT` | `5008` | Legacy module/config default; the `ax-bi mcp run` CLI defaults to `31421` |
...
| `FAB_API_KEY_ENABLED` | `True` | Register owner-scoped API-key management and API-key authentication |
| `MCP_API_KEY_ENABLED` | `None` | Accept FAB API keys on MCP; `None` follows `FAB_API_KEY_ENABLED` |
| `MCP_API_KEY_CREATE_URL` | `"/"` | Key-management URL shown after an invalid or expired API key |
...
- Confirm the port is not already in use: `lsof -i :31421`
...
- Confirm the URL path ends with `/mcp` (e.g., `http://localhost:31421/mcp`)
...
- **Require authentication** for every internet-facing deployment using a user-bound API key, validated JWT, or both; never set `MCP_DEV_USERNAME`
- **RBAC enforcement** -- The MCP server respects AX BI's role-based access control. Users can only access data their roles permit
...
- **Scoped JWTs** -- Use `MCP_REQUIRED_SCOPES` to limit JWT operations; FAB API keys continue to use owner RBAC and ACLs
