...
can be found under the `ax-bi-frontend/plugins` directory. Viz plugins are added to
the application in the `ax-bi-frontend/src/visualizations/presets/MainPreset.js`.
...
```bash
npm i -g yo
cd ax-bi-frontend/packages/generator-superset
npm i
npm link
```
...
To add the package to Superset, go to the `ax-bi-frontend` subdirectory in your
...
After this edit the `ax-bi-frontend/src/visualizations/presets/MainPreset.js`
...
found here](https://github.com/defai-digital/ax-bi/tree/main/scripts/tests)
...
```bash
# ax-bi-frontend/cypress-base is the base folder for everything Cypress-related
...
See [`ax-bi-frontend/cypress_build.sh`](https://github.com/defai-digital/ax-bi/blob/main/ax-bi-frontend/cypress_build.sh).
...
(pyproject.toml)[https://github.com/defai-digital/ax-bi/blob/main/pyproject.toml] file
