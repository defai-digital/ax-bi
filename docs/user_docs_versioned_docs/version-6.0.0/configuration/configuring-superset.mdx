...
[config.py module](https://github.com/defai-digital/ax-bi/blob/main/superset/config.py). The
...
# Alternatively you can set it with `AX_BI_SECRET_KEY` environment variable.
...
core [superset/config.py](https://github.com/defai-digital/ax-bi/blob/main/superset/config.py) that
...
in [superset/config.py](https://github.com/defai-digital/ax-bi/blob/main/superset/config.py)
...
Alternatively, you can set the secret key using `AX_BI_SECRET_KEY` environment variable:

On a Unix-based system, such as Linux or macOS, you can do so by running the following command in your terminal:

```bash
export AX_BI_SECRET_KEY=$(openssl rand -base64 42)
```
...
Save your `superset_config.py` with these values and then run `ax-bi re-encrypt-secrets`.
...
information. _Note that the development web server (`ax-bi run` or `flask run`) is not intended
...
- Customizing the [Flask entrypoint](https://github.com/defai-digital/ax-bi/blob/main/superset/app.py#L29)
...
[`SUPERSET_APP_ROOT`](https://github.com/defai-digital/ax-bi/blob/main/docker/.env),
...
```
If you encounter the [issue](https://github.com/apache/superset/issues/13243) of not being able to list users from the Superset main page settings, although a newly registered user has an `Admin` role, please re-run `ax-bi init` to sync the required permissions. Below is the command to re-run `ax-bi init` using docker compose.
```
docker-compose exec superset ax-bi init
```
