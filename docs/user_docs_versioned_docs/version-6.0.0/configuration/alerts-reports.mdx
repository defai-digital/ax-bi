...
If you are running a non-dev docker image, e.g., a stable release like `apache/superset:3.1.0`, that image does not include a headless browser.  Only the `ax_bi_worker` container needs this headless browser to browse to the target chart or dashboard.
...
8. Restart the service (or run `ax-bi init`) to pull in the new configuration.
...
- You must have a `celery beat` pod running. If you're using the chart included in the GitHub repository under [helm/superset](https://github.com/defai-digital/ax-bi/tree/main/helm/superset), you need to put `supersetCeleryBeat.enabled = true` in your values override.
...
You can find documentation about each field in the default `config.py` in the GitHub repository under [superset/config.py](https://github.com/defai-digital/ax-bi/blob/main/superset/config.py).
...
This is the best source of information about the problem.  In a docker compose deployment, you can do this with a command like `docker logs ax_bi_worker --since 1h`.
