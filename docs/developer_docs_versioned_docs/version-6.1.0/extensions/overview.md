...
Extension developers have access to pre-built UI components via `@ax-bi/core/components`. Browse all available components on the [UI Components](/developer-docs/components/) page and filter by **Extension Compatible** to see components available to extensions.
