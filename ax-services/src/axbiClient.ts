...
export interface AxBIPermissionClient {
  checkPermission(
...
export interface AxBIAnnotationListClient {
  listAnnotations(
...
export interface AxBIAnnotationLayerListClient {
...
export interface AxBIDashboardListClient {
  listDashboards(
...
export interface AxBIChartListClient {
  listCharts(
...
export interface AxBIDatabaseListClient {
  listDatabases(
...
export interface AxBIDatasetListClient {
  listDatasets(
...
export interface AxBIQueryListClient {
  listQueries(
...
export interface AxBIReportListClient {
  listReports(
...
export interface AxBIRoleListClient {
  listRoles(
...
export interface AxBIRlsListClient {
  listRlsFilters(
...
export interface AxBISavedQueryListClient {
  listSavedQueries(
...
export interface AxBITagListClient {
  listTags(
...
export interface AxBITaskListClient {
  listTasks(
...
export interface AxBIDependencyClient
  extends AxBIHealthClient,
    AxBIMetadataClient,
    AxBIAnnotationListClient,
    AxBIAnnotationLayerListClient,
    AxBIAssetSearchClient,
    AxBIPermissionClient,
    AxBIDashboardListClient,
    AxBIChartListClient,
    AxBIDatabaseListClient,
    AxBIDatasetListClient,
    AxBIQueryListClient,
    AxBIReportListClient,
    AxBIRoleListClient,
    AxBIRlsListClient,
    AxBISavedQueryListClient,
    AxBITagListClient,
    AxBITaskListClient {}
...
export class AxBIClient implements AxBIDependencyClient {
  ...
  constructor(private readonly config: ServiceConfig) {
    this.healthUrl = `${config.axbiBaseUrl}${config.axbiHealthPath}`;
    this.metadataUrl = `${config.axbiBaseUrl}${config.axbiMetadataPath}`;
    this.permissionUrl = `${config.axbiBaseUrl}${config.axbiPermissionPath}`;
  }
  ...
  private buildHeaders(
    ...
    if (this.config.axbiInternalToken !== undefined) {
      headers['authorization'] = `Bearer ${this.config.axbiInternalToken}`;
    }
  ...
  async checkHealth(correlationId?: string): Promise<DependencyHealth> {
    try {
      const response = await fetch(this.healthUrl, {
        ...
        signal: AbortSignal.timeout(this.config.axbiTimeoutMs),
      });
  ...
  >({
    ...
    toItem: (item: AxBIListItem) => TItem | undefined;
    buildResponse: (items: TItem[], totalCount: number) => TResponse;
  }): Promise<TResponse> {
    return this.fetchAxBIListOperation({
      url,
      correlationId,
      operationLabel: `${resourceLabel} list`,
      emptyResult: warnings => emptyResponse(request, warnings),
      onPayload: payload => {
        const items = extractAxBIResults(payload).map(toItem).filter(isDefined);
        const totalCount = extractAxBICount(payload, items.length);
  ...
  private async fetchAxBIListOperation<TResult>({
    url,
    correlationId,
    operationLabel,
    emptyResult,
    ...
    try {
      ...
      if (!response.ok) {
        return emptyResult([
          `${operationLabel} returned status ${response.status} from AxBI`,
        ]);
      }
  ...
  private async searchAssetType(
    ...
    return this.fetchAxBIListOperation<{
      ...
      onPayload: payload => ({
        assets: extractAxBIResults(payload)
  ...
  private buildAssetSearchUrl(
    ...
    const query = buildAxBIListQuery(
      filterColumn,
      request.query,
      request.limit,
  ...
  private buildAnnotationListUrl(request: AnnotationListRequest): string {
    const basePath = this.config.axbiAssetSearchPaths.annotationLayer.replace(
      /\/$/,
      '',
    );
    return this.buildAxBIQueryUrl(
      `${basePath}/${request.layerId}/annotation/`,
      buildAxBIPagedListQuery(request, listSearchColumns.annotation),
    );
  }

  private buildConfiguredPagedListUrl(
    pathKey: AxBIAssetSearchPathKey,
    request: AxBIListQueryRequest,
    searchColumn: string,
  ): string {
    return this.buildConfiguredListUrl(
      pathKey,
      buildAxBIPagedListQuery(request, searchColumn),
    );
  }
  ...
  private buildConfiguredListUrl(
    ...
    return this.buildAxBIQueryUrl(
      this.config.axbiAssetSearchPaths[pathKey],
      query,
    );
  }

  private buildAxBIQueryUrl(path: string, query: string): string {
    const url = new URL(`${this.config.axbiBaseUrl}${path}`);
...
interface AxBIListQueryRequest extends ListPaginationRequest {
  filters: ListFilter[];
  search?: string;
  orderColumn?: string;
...
function buildAxBIPagedListQuery(
  request: AxBIListQueryRequest,
  searchColumn: string,
): string {
...
function extractAxBIResults(payload: unknown): AxBIListItem[] {
  if (!isRecord(payload) || !Array.isArray(payload['result'])) {
    throw new Error('AxBI list response must include a result array');
  }

  return payload['result']
    .filter(isRecord)
    .map(item => item as AxBIListItem);
}

function extractAxBICount(payload: unknown, fallback: number): number {
  if (
    !isRecord(payload) ||
...
function toChartListItem(item: AxBIListItem): ChartListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
...
function toDatasetListItem(item: AxBIListItem): DatasetListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
  ...
  return omitUndefined<DatasetListItem>({
    ...
    isVirtual:
      typeof item.is_virtual === 'boolean'
        ...
        : item.type === 'virtual',
    databaseId:
      isAxBIId(item.database_id)
        ? item.database_id
        : isAxBIId(item.database?.id)
          ? item.database.id
          : undefined,
    uuid: optionalString(item.uuid),
    url: optionalString(item.url),
...
function toDatabaseListItem(item: AxBIListItem): DatabaseListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
...
function toSavedQueryListItem(
  ...
  return omitUndefined<SavedQueryListItem>({
    ...
    dbId: isAxBIId(item.db_id) ? item.db_id : undefined,
    schema: optionalString(item.schema),
...
function toAnnotationListItem(
  ...
  return omitUndefined<AnnotationListItem>({
    ...
    layerId: isAxBIId(item.layer_id) ? item.layer_id : fallbackLayerId,
  });
}

function toQueryListItem(item: AxBIListItem): QueryListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
  ...
  return omitUndefined<QueryListItem>({
    ...
    progress: isNonNegativeFiniteNumber(item.progress)
      ...
      : undefined,
    changedOn: optionalString(item.changed_on),
    userId: isAxBIId(item.user_id)
      ? item.user_id
      : isAxBIId(item.user?.id)
        ? item.user.id
        : undefined,
  });
}

function toReportListItem(item: AxBIListItem): ReportListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
  ...
  return omitUndefined<ReportListItem>({
    ...
    dashboardId:
      isAxBIId(item.dashboard_id) ? item.dashboard_id : undefined,
    chartId: isAxBIId(item.chart_id) ? item.chart_id : undefined,
...
function toRoleListItem(item: AxBIListItem): RoleListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
...
function toRlsListItem(item: AxBIListItem): RlsListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
...
function toTagListItem(item: AxBIListItem): TagListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
...
function toTaskListItem(item: AxBIListItem): TaskListItem | undefined {
  if (!isAxBIId(item.id)) {
    return undefined;
  }
...
function extractDatabaseName(item: AxBIListItem): string | undefined {
...
function isAxBIId(value: unknown): value is number {
...
function hasValidAuthorizationIds(request: unknown): boolean {
  ...
  return (
    isOptionalAxBIId(principal['userId']) &&
    isOptionalAxBIId(resource['id'])
  );
}
...
function isOptionalAxBIId(value: unknown): boolean {
  return value === undefined || isAxBIId(value);
}
