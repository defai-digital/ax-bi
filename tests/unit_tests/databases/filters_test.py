...
def test_database_filter(mocker: MockerFixture) -> None:
    ...
    assert (
        "'[' || dbs.database_name || '].(id:' || CAST(dbs.id AS VARCHAR) || ')'"
        in compiled_sql
    )
    assert "'[my_db].(id:42)', '[my_other_db].(id:43)'" in compiled_sql
    assert "dbs.database_name IN ('my_db', 'my_other_db', 'third_db')" in compiled_sql
