...
from axbi.axbi_typing import DbapiResult
from axbi.db_engine_specs.base import BaseEngineSpec
from axbi.result_set import (
    AxBIResultSet,
...
def test_column_names_as_bytes() -> None:
    ...
    result_set = AxBIResultSet(data, description, RedshiftEngineSpec)  # type: ignore
...
def test_timezone_series(mocker: MockerFixture) -> None:
    ...
    """
    logger = mocker.patch("axbi.result_set.logger")
...
def test_out_of_bounds_datetime_coerced_to_nat(mocker: MockerFixture) -> None:
    ...
    value is representable. AxBIResultSet must not raise or log an ERROR
...
def test_empty_column_names_get_synthetic_names() -> None:
    ...
    AxBIResultSet must replace empty column names with synthetic names
    so queries like ``SELECT COUNT(*) FROM t`` succeed on MSSQL.

    Regression test for https://github.com/defai-digital/ax-bi/issues/23848
    """
    data = [(42,)]
...
def test_empty_column_names_do_not_rename_explicit_synthetic_names() -> None:
    ...
    user-selected names that already look like AxBI fallbacks.
    """
    data = [(10, 20)]
    description = [
        ("", 3, None, None, None, None, None),
...
def test_json_data_type_preserved_as_objects() -> None:
    ...
    See: https://github.com/defai-digital/ax-bi/issues/25125
    """
...
def test_uuid_column_is_stringified() -> None:
    ...
    ``AxBIResultSet`` must stringify any Arrow extension type.
...
def test_stringify_extension_columns() -> None:
    ...
    ``AxBIResultSet`` and the semantic-layers mapper.
    """
    import uuid
...
def test_stringify_values_dict_and_list_produce_valid_json() -> None:
    ...
    parsed = axbi_json.loads(result[0])
    assert parsed == {"key": "value", "nested": {"a": 1}}
    parsed = axbi_json.loads(result[1])
    assert parsed == ["a", "b"]
...
def test_clickhouse_json_column_in_pa_table_is_valid_json() -> None:
    ...
    AxBIResultSet must serialize them with json.dumps (not str()) so that
    ...
    df_from_pa = AxBIResultSet.convert_table_to_df(result_set.pa_table)
    ...
    parsed0 = axbi_json.loads(val0)
    assert parsed0 == {"a": {"b": 42}, "c": [1, 2, 3], "d": "Hello, World!"}
    parsed1 = axbi_json.loads(val1)
    assert parsed1 == {"e": 5}
