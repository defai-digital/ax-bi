...
def test_validate_production_evidence_rejects_unselected_flag_workflows() -> None:
    """Production flag-state records must match the selected workflow scope."""
    ...
    assert validation["status"] == "failed"
    assert checks["production_flag_state"]["passed"] is False
    assert "selected workflow scope" in checks["production_flag_state"]["message"]
...
def test_validate_production_evidence_rejects_unselected_dashboard_workflows() -> None:
    """Dashboard workflow records must match the selected workflow scope."""
