...
def test_query_dao_stop_query_wrong_user(
    ...
    # Simulate a different non-admin user (user 2) attempting to stop user 1's query
    mocker.patch("axbi.daos.query.get_user_id", return_value=2)
    mocker.patch("axbi.security_manager.is_admin", return_value=False)
...
def test_query_dao_stop_query_admin_can_stop_others(
    mocker: MockerFixture, app: Any, session: Session
) -> None:
    """Admins must be able to cancel another user's runaway query."""
    from axbi import db
    from axbi.common.db_query_status import QueryStatus
    from axbi.models.core import Database
    from axbi.models.sql_lab import Query
    ...
    mocker.patch("axbi.daos.query.get_user_id", return_value=99)
    mocker.patch("axbi.security_manager.is_admin", return_value=True)
    mocker.patch("axbi.sql_lab.cancel_query", return_value=True)
