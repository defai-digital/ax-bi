...
def test_set_fast_cache_disabled(mock_app: MagicMock) -> None:
    ...
    with (
        patch("axbi.utils.fast_cache.current_app", mock_app),
        patch("axbi.utils.fast_cache._get_redis_client") as mock_get_client,
    ):
...
def test_set_fast_cache_writes_to_redis(mock_app: MagicMock) -> None:
    ...
    with (
        patch("axbi.utils.fast_cache.current_app", mock_app),
        patch("axbi.utils.fast_cache._get_redis_client", return_value=mock_redis),
    ):
...
def test_set_fast_cache_no_redis_client(mock_app: MagicMock) -> None:
    ...
    with (
        patch("axbi.utils.fast_cache.current_app", mock_app),
        patch("axbi.utils.fast_cache._get_redis_client", return_value=None),
    ):
        # Should not raise
        set_fast_cache("key1", '{"data":true}')
...
def test_get_fast_cache_hit(mock_app: MagicMock) -> None:
    ...
    from axbi.utils.fast_cache import get_fast_cache
...
def test_get_fast_cache_disabled(mock_app: MagicMock) -> None:
    ...
    with patch("axbi.utils.fast_cache.current_app", mock_app):
...
def test_custom_key_prefix(mock_app: MagicMock) -> None:
    ...
    from axbi.utils.fast_cache import get_fast_cache, set_fast_cache
