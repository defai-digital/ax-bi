...
def drop_from_schema(engine: Engine, schema_name: str):
    with engine.begin() as connection:
        schemas = connection.execute(text("SHOW SCHEMAS")).fetchall()  # noqa: F541
        ...
        for tv in tables_or_views:
            connection.execute(text(f"DROP TABLE IF EXISTS {schema_name}.{tv[0]}"))
            connection.execute(text(f"DROP VIEW IF EXISTS {schema_name}.{tv[0]}"))
...
def setup_presto_if_needed():
    ...
    if backend in {"presto", "hive"}:
        ...
        with database.get_sqla_engine() as engine:
            ...
            with engine.begin() as connection:
                connection.execute(text(f"DROP SCHEMA IF EXISTS {CTAS_SCHEMA_NAME}"))
                connection.execute(text(f"CREATE SCHEMA {CTAS_SCHEMA_NAME}"))

            drop_from_schema(engine, ADMIN_SCHEMA_NAME)
            with engine.begin() as connection:
                connection.execute(text(f"DROP SCHEMA IF EXISTS {ADMIN_SCHEMA_NAME}"))
                connection.execute(text(f"CREATE SCHEMA {ADMIN_SCHEMA_NAME}"))
...
def physical_dataset():
    ...
    with example_database.get_sqla_engine() as engine:
        ...
        time_column = quoter("time column with spaces")
        ...
        with engine.begin() as connection:
            connection.execute(
                ...
                {time_column} TIMESTAMP DEFAULT '1970-01-01 00:00:01'
                );
                """)
            )
            connection.execute(
