    ...
    def test_export_dataset_command_unicode_chars(self, mock_g) -> None:
        ...
        with examples_db.get_sqla_engine() as engine:
            with engine.begin() as connection:
                connection.execute(text("DROP TABLE IF EXISTS 中文"))
                connection.execute(text("CREATE TABLE 中文 AS SELECT 2 as col"))
        ...
        with examples_db.get_sqla_engine() as engine:
            with engine.begin() as connection:
                connection.execute(text("DROP TABLE 中文"))
        db.session.commit()
...
class TestCreateDatasetCommand(AxBITestCase):
    ...
    def test_create_dataset_command(self):
        ...
        with examples_db.get_sqla_engine() as engine:
            with engine.begin() as connection:
                connection.execute(
                    text("DROP TABLE IF EXISTS test_create_dataset_command")
                )
                connection.execute(
        ...
        with examples_db.get_sqla_engine() as engine:
            with engine.begin() as connection:
                connection.execute(text("DROP TABLE test_create_dataset_command"))
        db.session.commit()
