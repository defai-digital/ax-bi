...
AX BI is a GenAI-native business intelligence platform built on Apache Superset
...
helm install ax-bi helm/superset -f helm/superset/values-axbi.yaml
```
...
| mcp.command[2] | string | `". {{ .Values.configMountPath }}/superset_bootstrap.sh; exec ax-bi mcp run --host 0.0.0.0 --port {{ .Values.mcp.service.port }}"` |  |
