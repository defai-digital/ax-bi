...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  echo "Usage: ./scripts/tag_latest_release.sh [--dry-run] <GITHUB_TAG_NAME>"
  echo
  echo "Tag a release as latest when it is newer than the current latest tag."
  exit 0
fi
...
if [[ -z "$compare_result" ]]; then
    ...
    set_github_output "SKIP_TAG=false"
