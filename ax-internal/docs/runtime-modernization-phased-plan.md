...
- MCP annotation layer listing can shadow or serve from `ax-services` behind
  `TS_MCP_ORCHESTRATION` and `TS_ANNOTATION_LAYER_LIST_SERVING`, with Python
  fallback.
- MCP layer annotation listing can shadow or serve from `ax-services` behind
  `TS_MCP_ORCHESTRATION` and `TS_LAYER_ANNOTATION_LIST_SERVING`, with Python
  fallback.
- MCP query history listing can shadow or serve from `ax-services` behind
  `TS_MCP_ORCHESTRATION` and `TS_QUERY_LIST_SERVING`, with Python fallback and
  MCP-side SQL/error-text sanitization before sidecar results are returned.
...
- MCP saved query listing can shadow or serve from `ax-services` behind
  `TS_MCP_ORCHESTRATION` and `TS_SAVED_QUERY_LIST_SERVING`, with Python
  fallback.
- MCP report listing can shadow or serve from `ax-services` behind
...
- MCP task listing can shadow or serve from `ax-services` behind
  `TS_MCP_ORCHESTRATION` and `TS_TASK_LIST_SERVING`, with Python fallback.
...
- `superset runtime-modernization validate-production-evidence` checks a JSON
  ...
  exactly the workflows serving production traffic, requires compatibility
  reports to include inventory and benchmark details, requires Rust benchmark
  artifacts to include measured Python/Rust duration and throughput, separates
  the Phase 3 first-workflow serving gate from the Phase 5 two-workflow
  selective rollout gate, reports the enabled, approved, dashboard-required, and
  failing evidence check names, and can fail release gates when evidence is
  missing, failing, or approval does not exactly match the enabled production
  workflow scope.
