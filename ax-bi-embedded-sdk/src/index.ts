...
export type EmbedDashboardParams = {
  /** The id provided by the embed configuration UI in AxBI */
  id: string;
  /** The domain where AxBI can be located, with protocol, such as: https://ax-bi.example.com */
  axbiDomain: string;
   ...
   * to allow the host app to customize the URL. If not provided, AxBI's default URL is used. */
...
/**
 * Embeds a AxBI dashboard into the page using an iframe.
 */
export async function embedDashboard({
  ...
  function log(...info: unknown[]) {
    if (debug) {
      console.debug(`[ax-bi-embedded-sdk][dashboard ${id}]`, ...info);
    }
  }
  ...
  if (axbiDomain.endsWith('/')) {
    axbiDomain = axbiDomain.slice(0, -1);
  }
    ...
    return new Promise(resolve => {
      ...
      iframe.addEventListener('load', () => {
        ...
        resolve(
          new Switchboard({
            port: ourPort,
            name: 'ax-bi-embedded-sdk',
            debug,
          }),
        );
      });
      iframe.src = `${axbiDomain}/embedded/${id}${urlParamsString}`;
